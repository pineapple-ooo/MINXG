/*
 * agent_harness_evolve.c — TIDAL LOCK ARCHITECTURE: Self-Evolution Core Implementation
 * ============================================================================
 *
 ...
 *       agent_harness_evolve.c mem_pool.c agent_harness_arch.c \
 *       -lxxhash -lzstd -lm -lpthread \
 *       -o libagent_harness_evolve.so
 *
 *   On systems without xxhash: -DAgentHarness_NO_XXHASH (pure-C MurmurHash3 fallback)
 *   On systems without zstd:   -DAgentHarness_NO_ZSTD   (zlib fallback)
 */

#include "agent_harness_evolve.h"
#include "mem_pool.h"
...
int agent_harness_evolve_kgrams_v1(
    const uint8_t* text, size_t text_len, int k,
    char* out_grams, size_t out_cap, int* out_count
) {
    if (!text || !out_grams || !out_count) return AgentHarness_ERR_NULL_PTR;
    ...
    for (size_t i = 0; i <= text_len - (size_t)k; i++) {
        if (pos + (size_t)k + 1 > out_cap) return AgentHarness_ERR_BUFFER_TOO_SMALL;
...
int agent_harness_evolve_minhash_v1(
    const uint8_t* text, size_t text_len,
    uint64_t* sig, size_t sig_cap
) {
    if (!sig || sig_cap < AgentHarness_MINHASH_DIM) return AgentHarness_ERR_BUFFER_TOO_SMALL;
    if (!text || text_len == 0) {
        memset(sig, 0, AgentHarness_MINHASH_DIM * sizeof(uint64_t));
        return AgentHarness_OK;
    }

    /* Initialize all 64 slots to max uint64 */
    for (int i = 0; i < AgentHarness_MINHASH_DIM; i++) {
        sig[i] = UINT64_MAX;
    }
...
double agent_harness_evolve_jaccard_v1(
...
uint64_t agent_harness_evolve_simhash_v1(
    const uint8_t* text, size_t text_len
) {
...
int agent_harness_evolve_hamming_v1(uint64_t a, uint64_t b) {
...
double agent_harness_evolve_js_divergence_v1(
    ...
    for (int i = 0; i < n_a; i++) {
        int d = agent_harness_evolve_hamming_v1(sigs_a[i], centroid_a) >> 2;
        if (d < 16) hist_a[d] += 1.0;
    }
    for (int i = 0; i < n_b; i++) {
        int d = agent_harness_evolve_hamming_v1(sigs_b[i], centroid_b) >> 2;
...
double agent_harness_evolve_ncd_v1(
...
int agent_harness_evolve_ncd_matrix_v1(
    const uint8_t** seqs, const size_t* lengths, int n,
    double* matrix
) {
    if (!seqs || !lengths || !matrix || n < 1) return AgentHarness_ERR_NULL_PTR;
    if (n > 500) return AgentHarness_ERR_BUFFER_TOO_SMALL; /* O(n^2) safeguard */
    ...
    if (!c_len || !compressed) {
        ...
        return AgentHarness_ERR_MEMORY;
    }
...
int agent_harness_evolve_spectral_v1(
    const double* adj, int n,
    double* eigenvals, int eigen_cap
) {
    if (!adj || !eigenvals || n < 1) return AgentHarness_ERR_NULL_PTR;
    if (eigen_cap < 1) return AgentHarness_ERR_BUFFER_TOO_SMALL;
    ...
    if (!D) return AgentHarness_ERR_MEMORY;
    for (int i = 0; i < n; i++) {
        double sum = 0.0;
    ...
    if (!L) { free(D); return AgentHarness_ERR_MEMORY; }
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i == j) {
                L[i * n + j] = 1.0;
...
int agent_harness_evolve_momentum_v1(
    ...
    if (!positions || !timestamps || !out_velocity || !out_speed || !out_confidence)
        return AgentHarness_ERR_NULL_PTR;
    if (n < 2) {
...
int agent_harness_evolve_drift_v1(
...
int agent_harness_evolve_topology_v1(
    ...
    if (!parent) return AgentHarness_ERR_MEMORY;
...
double agent_harness_evolve_euclidean_v1(const double* a, const double* b, int dim) {
...
double agent_harness_evolve_cosine_v1(const double* a, const double* b, int dim) {
    if (!a || !b || dim < 1) return 0.0;
...
double agent_harness_evolve_entropy_v1(const double* p, int n) {
    if (!p || n < 1) return 0.0;
    double sum = 0.0;
    for (int i = 0; i < n; i++) {
...
const char* agent_harness_evolve_abi_version(void) {
    return "tidal_lock_v1";
}

int agent_harness_evolve_selftest(void) {
    int failures = 0;
    ...
    /* Test 1: MinHash on known text */
    {
        ...
        uint64_t sig[64];
        int rc = agent_harness_evolve_minhash_v1((const uint8_t*)test, strlen(test), sig, 64);
        if (rc != AgentHarness_OK) { fprintf(stderr, "FAIL: minhash returned %d\n", rc); failures++; }
        int nonzero = 0;
    ...
    /* Test 2: MinHash reproducibility */
    {
        ...
        agent_harness_evolve_minhash_v1((const uint8_t*)"test", 4, sig1, 64);
        agent_harness_evolve_minhash_v1((const uint8_t*)"test", 4, sig2, 64);
    ...
    {
        uint64_t sig1[64], sig2[64];
        agent_harness_evolve_minhash_v1((const uint8_t*)"same text", 9, sig1, 64);
        agent_harness_evolve_minhash_v1((const uint8_t*)"same text", 9, sig2, 64);
        double j = agent_harness_evolve_jaccard_v1(sig1, sig2, 64);
        if (j < 0.9) { fprintf(stderr, "FAIL: jaccard identical = %.4f\n", j); failures++; }
    }

    /* Test 4: SimHash */
    {
        uint64_t sh = agent_harness_evolve_simhash_v1((const uint8_t*)"hello world", 11);
        if (sh == 0) { fprintf(stderr, "FAIL: simhash zero\n"); failures++; }
    }

    /* Test 5: Hamming distance */
    {
        int d = agent_harness_evolve_hamming_v1(0xFFFFFFFFFFFFFFFFULL, 0ULL);
        if (d != 64) { fprintf(stderr, "FAIL: hamming FF vs 00 = %d\n", d); failures++; }
    }

    /* Test 6: NCD (identical -> ~0, small strings have zstd overhead) */
    {
        double d = agent_harness_evolve_ncd_v1(
    ...
    {
        double a[] = {0, 3}, b[] = {4, 0};
        double d = agent_harness_evolve_euclidean_v1(a, b, 2);
    ...
    {
        double a[] = {1, 0}, b[] = {0, 1};
        double c = agent_harness_evolve_cosine_v1(a, b, 2);
    ...
    {
        double p[] = {0.5, 0.5};
        double e = agent_harness_evolve_entropy_v1(p, 2);
        if (fabs(e - log(2.0)) > 0.01) { fprintf(stderr, "FAIL: entropy uniform = %.4f\n", e); failures++; }
    }

    /* Test 10: ABI version */
    {
        const char* v = agent_harness_evolve_abi_version();
