...
/* Forward decl */
int64_t agent_harness_bmh_search(const uint8_t* h, size_t hl, const uint8_t* n, size_t nl);
...
int main() {
    ...
    int64_t lib = agent_harness_bmh_search(h, 11, n, 5);
    printf("agent_harness_bmh_search = %ld\n", (long)lib);

    /* Also test with explicit args */
    int64_t r2 = agent_harness_bmh_search((uint8_t*)h, (size_t)11, (uint8_t*)n, (size_t)5);
