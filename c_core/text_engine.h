...
#ifndef AgentHarness_TEXT_ENGINE_H
#define AgentHarness_TEXT_ENGINE_H
 ...
 */
int64_t agent_harness_memmem(const uint8_t* haystack, size_t hay_len,
                     const uint8_t* needle,  size_t ndl_len);
int64_t agent_harness_memrmem(const uint8_t* haystack, size_t hay_len,
                      const uint8_t* needle,  size_t ndl_len);
int     agent_harness_memcnt(const uint8_t* haystack, size_t hay_len,
                     const uint8_t* needle,  size_t ndl_len);

/* ─── String transforms (in-place, zero-alloc) ──────────────────────────── */
size_t agent_harness_str_lower(char* str, size_t len);
size_t agent_harness_str_upper(char* str, size_t len);
size_t agent_harness_str_trim(char* str, size_t len);  /* returns new len */
...
typedef struct {
    ...
    int         col;
    char        delim;
} agent_harness_csv_reader_t;

agent_harness_csv_reader_t agent_harness_csv_open(const char* data, size_t len, char delim);
/*
 * Returns length of cell written to out_buf (0 = empty cell, -1 = EOF).
 * out_buf must be at least out_cap bytes.
 */
int agent_harness_csv_next_cell(agent_harness_csv_reader_t* r, char* out_buf, size_t out_cap);

/*
 * agent_harness_csv_count — quick row/col count without parsing content.
 * out_rows/out_cols may be NULL.
 */
void agent_harness_csv_count(const char* data, size_t len, char delim,
                     int* out_rows, int* out_cols);

/* ─── Glob matching (fnmatch-lite, zero-alloc) ─────────────────────────── */
bool agent_harness_fnmatch(const char* pattern, const char* str);
bool agent_harness_fnmatch_caseless(const char* pattern, const char* str);

/* ─── Unicode helpers ──────────────────────────────────────────────────── */
int  agent_harness_utf8_codepoint_count(const char* str, size_t len);
bool agent_harness_utf8_is_valid(const char* str, size_t len);
int  agent_harness_utf8_grapheme_count(const char* str, size_t len);

/* ─── String utilities (new in v0.0.2) ──────────────────────────────────── */

/* slugify: convert to lowercase, strip non-word chars, collapse '-' and spaces */
size_t agent_harness_slugify(const char* input, size_t in_len, char* out_buf, size_t out_cap);

/* truncate: cut to max_len, append suffix if truncated. Returns final len. */
size_t agent_harness_truncate(const char* input, size_t in_len, size_t max_len,
...
size_t agent_harness_word_freq_hash(const char* input, size_t in_len,
...
int agent_harness_extract_urls(const char* input, size_t in_len,
                       char* out_buf, size_t out_cap, int max_urls);

/* extract_emails: find all email addresses, return count. */
int agent_harness_extract_emails(const char* input, size_t in_len,
                         char* out_buf, size_t out_cap, int max_emails);

/* extract_hashtags: find all #tags, return count. */
int agent_harness_extract_hashtags(const char* input, size_t in_len,
...
size_t agent_harness_normalize_ws(const char* input, size_t in_len,
...
int agent_harness_base_convert(const char* number, int base_fr, int base_to,
                       char* out_buf, size_t out_cap);

/* statistics: compute count, mean, std, median, min, max, sum from flat
 * array of doubles. Returns 0 on success, -1 on error. */
int agent_harness_statistics(const double* values, size_t count,
...
#endif /* AgentHarness_TEXT_ENGINE_H */