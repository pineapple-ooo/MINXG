/*
 * agent_harness_arch.c — Core C implementations: thread pool, ring buffer, buffer ops
 *
...
const char* agent_harness_strerror(agent_harness_err_t err) {
    switch (err) {
        case AgentHarness_OK:                    return "OK";
        case AgentHarness_ERR_NULL_PTR:          return "NULL pointer";
        case AgentHarness_ERR_BUFFER_TOO_SMALL:  return "Buffer too small";
        case AgentHarness_ERR_IO:                return "I/O error";
        case AgentHarness_ERR_CRYPTO:            return "Crypto error";
        case AgentHarness_ERR_ENCODING:          return "Encoding error";
        case AgentHarness_ERR_PARSE:             return "Parse error";
        case AgentHarness_ERR_BOUNDS:            return "Out of bounds";
        case AgentHarness_ERR_TIMEOUT:           return "Timeout";
        case AgentHarness_ERR_PERMISSION:        return "Permission denied";
        case AgentHarness_ERR_THREAD:            return "Thread error";
        case AgentHarness_ERR_MEMORY:            return "Memory error";
        case AgentHarness_ERR_NOT_FOUND:         return "Not found";
        case AgentHarness_ERR_ALREADY_EXISTS:    return "Already exists";
        case AgentHarness_ERR_INVALID_STATE:     return "Invalid state";
        case AgentHarness_ERR_NOT_IMPLEMENTED:   return "Not implemented";
...
agent_harness_buf_t agent_harness_buf_from_bytes(const uint8_t* src, size_t len) {
    agent_harness_buf_t buf = agent_harness_buf_new(len ? len : 1);
    if (buf.data && len > 0) {
...
agent_harness_buf_t agent_harness_buf_from_cstr(const char* str) {
    ...
    return agent_harness_buf_from_bytes((const uint8_t*)str, len);
}
...
agent_harness_err_t agent_harness_buf_reserve(agent_harness_buf_t* buf, size_t new_cap) {
    if (!buf) return AgentHarness_ERR_NULL_PTR;
    if (new_cap <= buf->cap) return AgentHarness_OK;
    uint8_t* new_data = (uint8_t*)realloc(buf->data, new_cap);
    if (!new_data) return AgentHarness_ERR_MEMORY;
...
agent_harness_err_t agent_harness_buf_append(agent_harness_buf_t* buf, const uint8_t* src, size_t len) {
    if (!buf || !src) return AgentHarness_ERR_NULL_PTR;
    if (len == 0) return AgentHarness_OK;
    if (buf->len + len > buf->cap) {
        size_t new_cap = buf->cap ? buf->cap * 2 : 64;
        while (new_cap < buf->len + len) new_cap *= 2;
        agent_harness_err_t e = agent_harness_buf_reserve(buf, new_cap);
        if (e != AgentHarness_OK) return e;
    }
...
#define AgentHarness_MAX_THREADS    64
#define AgentHarness_POOL_QUEUE_SZ 1024

typedef struct {
    agent_harness_work_fn fn;
    void*         arg;
    atomic_bool   used;
} pool_item_t;

typedef struct {
    pool_item_t  queue[AgentHarness_POOL_QUEUE_SZ];
    atomic_int   head;
    atomic_int   tail;
    atomic_int   pending;
    int          num_threads;
    pthread_t    threads[AgentHarness_MAX_THREADS];
    atomic_bool  running;
...
static void* pool_worker(void* arg) {
    ...
    while (atomic_load(&p->running)) {
        ...
        agent_harness_work_fn fn = item->fn;
        void* fn_arg   = item->arg;
        item->used = false;
        int new_head = (head + 1) % AgentHarness_POOL_QUEUE_SZ;
...
agent_harness_thread_pool_t agent_harness_thread_pool_create(int num_threads) {
    if (num_threads <= 0 || num_threads > AgentHarness_MAX_THREADS) return NULL;
...
agent_harness_err_t agent_harness_thread_pool_submit(agent_harness_thread_pool_t pool,
    ...
    if (!pool || !fn) return AgentHarness_ERR_NULL_PTR;
    pool_impl_t* p = (pool_impl_t*)pool;

    pthread_mutex_lock(&p->mutex);
    int tail = atomic_load(&p->tail);
    int next = (tail + 1) % AgentHarness_POOL_QUEUE_SZ;
    if (next == atomic_load(&p->head)) {
        pthread_mutex_unlock(&p->mutex);
        return AgentHarness_ERR_BUFFER_TOO_SMALL; // queue full
    }
...
agent_harness_err_t agent_harness_thread_pool_wait(agent_harness_thread_pool_t pool) {
    if (!pool) return AgentHarness_ERR_NULL_PTR;
    pool_impl_t* p = (pool_impl_t*)pool;
...
agent_harness_err_t agent_harness_ring_push(agent_harness_ring_t ring, const uint8_t* data, size_t len) {
    if (!ring || !data || len == 0) return AgentHarness_ERR_NULL_PTR;
    ring_impl_t* r = (ring_impl_t*)ring;
    if (len + sizeof(uint32_t) > agent_harness_ring_writable(ring))
        return AgentHarness_ERR_BUFFER_TOO_SMALL;
...
agent_harness_err_t agent_harness_ring_pop(agent_harness_ring_t ring, uint8_t* out, size_t out_cap, size_t* out_len) {
    if (!ring || !out || !out_len) return AgentHarness_ERR_NULL_PTR;
    ring_impl_t* r = (ring_impl_t*)ring;
    if (agent_harness_ring_readable(ring) < 4) return AgentHarness_ERR_BUFFER_TOO_SMALL;

    uint64_t pos = atomic_load(&r->read_seq) & r->mask;
    uint32_t hdr = 0;
    memcpy(&hdr, r->buf + pos, 4);
    if (hdr > out_cap) return AgentHarness_ERR_BUFFER_TOO_SMALL;
...
size_t agent_harness_ring_writable(agent_harness_ring_t ring) {
    if (!ring) return 0;
    ring_impl_t* r = (ring_impl_t*)ring;
    return r->cap - agent_harness_ring_readable(ring) - 1;
}
