# agent_harness R geometry adapter -- statistical geometry computations.
#
...
agent_harness_point_cloud_stats <- function(points) {
...
agent_harness_pca_orientation <- function(points) {
  pts <- as.matrix(points)
...
agent_harness_kde_boundary <- function(points, bandwidth = NULL) {
  pts <- as.matrix(points)
  if (ncol(pts) != 2) {
...
agent_harness_polygon_area <- function(points) {
  pts <- as.matrix(points)
