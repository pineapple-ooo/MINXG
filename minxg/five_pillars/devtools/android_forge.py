"""agent_harness/five_pillars/devtools/android_forge.py — Heavy-weight Android forge worker.
...
def _default_kivy_entry() -> str:
    return textwrap.dedent(
        """\
        # AgentHarness APK entry point (kivy)
        from kivy.app import App
        from kivy.uix.label import Label

        class HelloKivy(App):
            def build(self):
                return Label(text="Hello from AgentHarness-built APK")
...
class AndroidForgeWorker(BaseWorker):
    ...
    * ``apk_scaffold`` — write a fresh agent_harness-buildable project tree.
    ...
    * ``apk_status`` — what state is package.json in / last log?
    ...
    @tool(
        description=(
            "Write a fresh agent_harness-buildable project tree to ``root_path`` from a "
    ...
    @tool(
        description=(
            ...
            "stored agent_harness-manifest.json. Idempotent — safe to re-run."
        ),
        category="deploy",
    )
    async def apk_spec(self, root_path: str) -> Dict[str, Any]:
        ...
        spec_lines = [
            ...
            "title = " + str(blueprint.get("title", "AgentHarness APP")),
            "package.name = " + str(blueprint.get("package", "ai.agent_harness.demo")),
            "package.domain = org",
            "source.dir = .",
    ...
    async def apk_preflight(self, root_path: str) -> Dict[str, Any]:
        ...
        entry = root / "main.py"
        if not entry.exists():
            entry = root / "agent_harness_main.py"
        checks.append({
            "name": "entry_script",
            "ok": entry.exists(),
            "value": str(entry),
    ...
    async def apk_status(self, root_path: str) -> Dict[str, Any]:
        ...
        manifest_file = root / "agent_harness-manifest.json"
        if not manifest_file.exists():
            return {"status": "error", "error": "no agent_harness-manifest.json"}
    ...
    async def apk_asset_pack(
        ...
        pkg = blueprint.get("package", "ai.agent_harness.demo")[:100]
        title = blueprint.get("title", "AgentHarness Demo")[:80]
        wrote = []
        ...
        themes.write_text(textwrap.dedent("""\
            <?xml version="1.0" encoding="utf-8"?>
            <resources>
                <style name="Theme.AgentHarness" parent="Theme.Material3.DayNight.NoActionBar">
...
def _compose_kotlin(name: str, components: List[str]) -> str:
    return textwrap.dedent(f"""\
        package ai.agent_harness.{name.lower()}
...
_APK_STUDIO_NOTICE = (
    ...
    "exclusive responsibility — AgentHarness and its contributors accept NO "
    "liability."
)
