...
class QuadForgeWorker(BaseWorker):
    ...
    async def _build_cargo(self, src: Path, prof: TargetProfile) -> Dict[str, Any]:
        ...
        await _run(["rustup", "target", "add", prof.rust_target], cwd=src, env=env, timeout=TIMEOUT_SUBPROCESS_BUILD)
    ...
    async def _build_cmake(self, src: Path, prof: TargetProfile) -> Dict[str, Any]:
        ...
        await _run(["cmake", "-B", str(bd), "-S", str(src)], cwd=bd, env=env, timeout=TIMEOUT_SUBPROCESS_BUILD)
