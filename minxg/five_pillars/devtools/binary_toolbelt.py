...
Integrates MIT-licensed reverse engineering tools that go beyond APK:
    ...
    3. AgentHarness's own Rust/C++ FFI deobfuscation passes (built-in).

Hermes Agent has no native binary analysis at this depth.  AgentHarness now
...
class BinaryToolbeltWorker(BaseWorker):
    ...
    # ── AgentHarness-native binary entropy scanner ────────────────────────
    ...
    async def binary_entropy_scan(
        ...
        Uses AgentHarness Rust core (FFT, entropy, autocorr) — zero external deps.
        """
        bp = Path(binary_path)
        if not bp.exists():
            return {"status": "error", "error": f"Binary not found: {binary_path}"}
        ...
        return {
            "status": "ok",
            "tool": "AgentHarness Rust FFI (signal.rs)",
            "binary": str(bp),
    ...
    # ── AgentHarness-native PE/ELF header parser ──────────────────────────

    @tool(
        description="Parse PE/ELF/Mach-O headers using AgentHarness's C++ core (no external deps).",
        category="reverse",
    )
    async def binary_header_parse(
        ...
        """Parse executable headers via AgentHarness C++ json_fast.
        ...
        return {
            "status": "ok",
            "tool": "AgentHarness C++ core (zero-deps)",
            "binary": str(bp),
            **result,
            "_legal_disclaimer": _LEGAL,
        }
