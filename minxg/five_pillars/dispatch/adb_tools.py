...
class AdbWorker(BaseWorker):
    ...
    def _adb_shell(self, command: str, device: str = "") -> Dict[str, Any]:
        return _adb(["shell", command] if not device else ["-s", device, "shell", command],
                    timeout=TIMEOUT_SUBPROCESS_TOOL)
    ...
    def _adb_uninstall(self, package: str, device: str = "") -> Dict[str, Any]:
        return _adb(["uninstall", package], timeout=TIMEOUT_SUBPROCESS_TOOL)

    def _adb_push(self, src: str, dst: str, device: str = "") -> Dict[str, Any]:
        return _adb(["push", src, dst], timeout=TIMEOUT_SUBPROCESS_TOOL)
    ...
    def _adb_pm_list(self, filter: str = "", device: str = "") -> Dict[str, Any]:
        ...
        if filter:
            cmd.append(filter)
        r = _adb(cmd, timeout=TIMEOUT_SUBPROCESS_QUICK)
        if r["status"] == "success":
    ...
    def _adb_am_start(self, package: str, activity: str = "",
        ...
        return _adb(["shell", "am", "start", "-n", target], timeout=TIMEOUT_SUBPROCESS_QUICK)

    def _adb_am_force_stop(self, package: str, device: str = "") -> Dict[str, Any]:
        return _adb(["shell", "am", "force-stop", package], timeout=TIMEOUT_SUBPROCESS_QUICK)
    ...
    def _adb_tcpip(self, port: int = 5555) -> Dict[str, Any]:
        return _adb(["tcpip", str(port)], timeout=TIMEOUT_SUBPROCESS_QUICK)

    def _adb_connect(self, host: str, port: int = 5555) -> Dict[str, Any]:
        return _adb(["connect", f"{host}:{port}"], timeout=TIMEOUT_SUBPROCESS_QUICK)

    def _adb_disconnect(self, host: str = "") -> Dict[str, Any]:
        return _adb(["disconnect", host] if host else ["disconnect"], timeout=TIMEOUT_SUBPROCESS_QUICK)

    def _adb_bugreport(self, device: str = "") -> Dict[str, Any]:
        return _adb(["bugreport"], timeout=TIMEOUT_SUBPROCESS_TOOL)
