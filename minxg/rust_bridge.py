...
class RustLib:
    ...
    def _sign(self):
        ...
        Missing symbols are skipped gracefully so the bridge works
        with a subset of the Rust library.
        """
        L = self.lib
        def _bind(name, **kwargs):
            try:
                fn = getattr(L, name)
                fn.argtypes = kwargs.get("argtypes", [])
                fn.restype = kwargs.get("restype", None)
                return fn
        ...
        L.rust_core_version.restype = c_void_p
        L.rust_core_free.argtypes = [c_void_p]
        L.rust_core_free.restype = None
        L.rust_math_operator_count.argtypes = []
        L.rust_math_operator_count.restype = c_uint32

        # GA — skip missing symbols
        _bind("ga_geometric_product", argtypes=[POINTER(Multivector), POINTER(Multivector), POINTER(Multivector)], restype=c_int32)
        _bind("ga_outer_product", argtypes=[POINTER(Multivector), POINTER(Multivector), POINTER(Multivector)], restype=c_int32)
        _bind("ga_inner_product", argtypes=[POINTER(Multivector), POINTER(Multivector), POINTER(c_double)], restype=c_int32)
        _bind("ga_rotor_apply", argtypes=[POINTER(c_double * 4), POINTER(c_double * 3), POINTER(c_double * 3)], restype=c_int32)

        # Driver
        _bind("driver_rk4_step", argtypes=[c_void_p, POINTER(StateVector), POINTER(c_double), c_uint32, c_double, c_double], restype=c_int32)
        _bind("driver_rk45_step", argtypes=[c_void_p, POINTER(StateVector), POINTER(c_double), c_uint32, c_double, c_double, c_double, c_double, c_double, c_double, POINTER(c_double)], restype=c_int32)

        # Chaos
        _bind("chaos_logistic_map", argtypes=[c_double, c_double], restype=c_double)
        _bind("chaos_lorenz_integrate", argtypes=[POINTER(c_double * 3), c_double, c_double, c_double, c_double, c_uint32], restype=c_int32)
        _bind("rust_logistic_map", argtypes=[c_double, c_double, c_int32], restype=c_double)
        _bind("rust_lorenz", argtypes=[c_double, c_double, c_double, c_double, c_double, c_double, c_double, c_int32, POINTER(c_double)], restype=None)

        # Fiber
        _bind("fiber_curvature", argtypes=[POINTER(ConnectionForm), POINTER(c_double * 16)], restype=c_int32)
        _bind("fiber_parallel_transport", argtypes=[POINTER(ConnectionForm), c_double, POINTER(c_double * 4)], restype=c_double)

        # SymbDiff
        _bind("symbdiff_jet_add", argtypes=[POINTER(Jet), POINTER(Jet), POINTER(Jet)], restype=c_int32)
        _bind("symbdiff_jet_mul", argtypes=[POINTER(Jet), POINTER(Jet), POINTER(Jet)], restype=c_int32)
        _bind("symbdiff_jet_exp", argtypes=[POINTER(Jet), POINTER(Jet)], restype=c_int32)

        # MemPool
        _bind("mempool_create", argtypes=[c_uint64], restype=c_void_p)
        _bind("mempool_free", argtypes=[c_void_p], restype=None)
        _bind("mempool_alloc", argtypes=[c_void_p, c_uint64], restype=c_uint64)
        _bind("mempool_free_slot", argtypes=[c_void_p, c_uint64], restype=c_int32)
        _bind("mempool_write", argtypes=[c_void_p, c_uint64, POINTER(c_uint8), c_uint64], restype=c_uint64)
        _bind("mempool_read", argtypes=[c_void_p, c_uint64, POINTER(c_uint8), c_uint64], restype=c_uint64)
        _bind("mempool_stats", argtypes=[c_void_p, POINTER(c_uint64)], restype=c_int32)

        # math_ops
        _bind("vec_mean", argtypes=[POINTER(c_double), c_uint32], restype=c_double)
        _bind("vec_std", argtypes=[POINTER(c_double), c_uint32], restype=c_double)
        _bind("vec_minmax", argtypes=[POINTER(c_double), c_uint32, POINTER(c_double), POINTER(c_double)], restype=c_int32)
        _bind("vec_correlation", argtypes=[POINTER(c_double), POINTER(c_double), c_uint32], restype=c_double)
        _bind("vec_stats", argtypes=[POINTER(c_double), c_uint32, POINTER(c_double), POINTER(c_double), POINTER(c_double), POINTER(c_double)], restype=c_int32)
        _bind("mat_mul", argtypes=[POINTER(c_double), c_uint32, c_uint32, POINTER(c_double), c_uint32, c_uint32, POINTER(c_double)], restype=c_int32)

        # str_ops
        _bind("str_hash64", argtypes=[POINTER(c_uint8), c_uint32], restype=c_uint64)
        _bind("str_count", argtypes=[POINTER(c_uint8), c_uint32, c_uint8], restype=c_uint32)
        _bind("str_lower_ascii", argtypes=[POINTER(c_uint8), c_uint32], restype=c_int32)
        _bind("str_utf8_validate", argtypes=[POINTER(c_uint8), c_uint32], restype=c_int32)
        _bind("vec_sum_sq", argtypes=[POINTER(c_double), c_uint32], restype=c_double)
        _bind("vec_dot", argtypes=[POINTER(c_double), POINTER(c_double), c_uint32], restype=c_double)
        _bind("vec_normalize", argtypes=[POINTER(c_double), c_uint32], restype=c_int32)
        _bind("vec_fused_sqadd_sum", argtypes=[POINTER(c_double), c_uint32], restype=c_double)
...
def logistic_map(r: float, x0: float, steps: int = 100) -> float:
    rl = RustLib.get()
    return rl.lib.rust_logistic_map(r, x0, steps)


def lorenz_integrate(state_xyz: List[float], sigma: float, rho: float, beta: float, dt: float, steps: int) -> List[float]:
    rl = RustLib.get()
    out = (c_double * 3)()
    rl.lib.rust_lorenz(state_xyz[0], state_xyz[1], state_xyz[2], sigma, rho, beta, dt, steps, out)
