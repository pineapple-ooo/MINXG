...
AgentHarness ToolChainExecutor: LLM emits a *plan* (JSON array of steps) → executor
...
LLM generates this plan once. AgentHarness executes it. One AI call. Done.
"""
...
class ToolRegistry:
    ...
    Tools are registered via the standard AgentHarness @tool decorator.
...
def chain(steps: List[Dict]):
    ...
    AgentHarness will execute it in one shot.
    """
    def decorator(fn):
