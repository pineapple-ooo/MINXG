...
def bundled_skills_dir() -> Path:
    """AgentHarness's own shipped skills (read-only, ships with the package)."""
...
def load_catalog(source: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    if source.startswith("http://") or source.startswith("https://"):
        ...
        try:
            with urllib.request.urlopen(source, timeout=TIMEOUT_HTTP_SKILL_FETCH) as resp:
...
def _git_clone_shallow(url: str, dest: Path, subdir: str = "") -> Path:
    try:
        result = subprocess.run(
            ...
            capture_output=True, text=True, timeout=TIMEOUT_SUBPROCESS_BUILD,
        )
