...
def _resolve_termux_api_bin() -> Optional[str]:
    """Return the best available termux-api executable path."""
    return shutil.which("termux-api") or (TERMUX_API_BIN if os.path.exists(TERMUX_API_BIN) else None)


def _termux_api_input_available() -> bool:
    termux_bin = _resolve_termux_api_bin()
    if not termux_bin:
        return False
    try:
        r = subprocess.run([termux_bin, "--help"], capture_output=True, text=True, timeout=5)
...
def tap(x: int, y: int, *, serial: str = "", source: str = "adb") -> dict:
    ...
    if source == "adb":
        ...
        termux_bin = _resolve_termux_api_bin()
        if not termux_bin:
            out["error"] = "termux-api not available"
            return out
        r = subprocess.run(
            [termux_bin, "input", str(x), str(y), "tap"],
            capture_output=True, text=True, timeout=10)
