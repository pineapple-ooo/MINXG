...
class MCPBridge:
    """Bridge AgentHarness's tool registry to external MCP servers.
    ...
    async def _mcp_list_tools(self, ep: MCPEndpoint) -> List[Dict[str, Any]]:
        """Stub — real impl is planned for a future release with aiohttp/stdio."""
        return []
    ...
    def _make_proxy(self, ep: MCPEndpoint, tool_name: str) -> Callable:
        async def proxy(**kwargs):
            return {"status": "ok", "mcp_bridge": True,
                    ...
                    "note": "MCP bridge stub — full JSON-RPC is planned for a future release"}
        return proxy
...
class ACPAdapter:
    ...
    AgentHarness's adapter connects to a running hermes-agent instance
    and exposes its tool surface as AgentHarness-native tools.
    """
    ...
    async def list_agent_tools(self, peer_name: str = "hermes-agent") -> List[Dict[str, Any]]:
        """ACP stub — real gRPC is planned for a future release."""
        return [{
