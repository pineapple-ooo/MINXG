"""
AgentHarness MCP Server — Expose AgentHarness workers as MCP tools

Connect Claude Code, Cursor, ChatGPT, and any MCP-compatible client
to AgentHarness's 70+ workers: file I/O, network, crypto, math, polyglot, and more.
...
class WorkerRegistry:
    """Lazy-load AgentHarness workers and expose them as MCP tools."""
    ...
    def load(self) -> None:
        if self._loaded or not _AgentHarness_AVAILABLE:
            return
        self._loaded = True
...
if FastMCP is not None:
    mcp = FastMCP(
        name=SERVER_NAME,
        instructions=f"""
AgentHarness MCP Server v{SERVER_VERSION}
...
def main() -> None:
    ...
    if transport == "http":
        ...
        print(f"Starting AgentHarness MCP Server on {host}:{port} (HTTP)")
        mcp.run(transport="streamable-http", host=host, port=port)
    else:
        print("Starting AgentHarness MCP Server (stdio)")
        mcp.run()
