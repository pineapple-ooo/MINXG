"""
AgentHarness NexusOrchestrator v3.3.3
...
from multiling.session_store import SessionStore
from multiling.constants import (
    MAX_MEMORY_FACTS, MAX_SESSIONS, MAX_TOOL_CALLS_PER_TURN,
    TIMEOUT_HTTP, TIMEOUT_AIOHTTP_TOTAL, TIMEOUT_AIOHTTP_KEEPALIVE,
    TIMEOUT_STREAMING, HALF_MINUTE, KiB, DEFAULT_MAX_TOKENS,
...
def _resolve_log_level() -> int:
    ...
    ``AgentHarness_LOG_LEVEL=INFO`` / ``DEBUG`` for diagnostics.
    """
    env = os.environ.get("AgentHarness_LOG_LEVEL", "").strip().upper()
...
class EpisodicMemory(MemoryLayer):
    ...
    Intentionally *not* a @dataclass — the hand-written __init__ supersedes
    MemoryLayer's dataclass-generated __init__ and sets up the turn index
    explicitly.
    """
...
class SemanticMemory:
    ...
    def __init__(self, max_facts: int = MAX_MEMORY_FACTS):
...
class ContextCompressor:
    ...
    def compress_messages(self, messages: List[Dict], preserve_system: bool = True) -> List[Dict]:
        ...
        if older:
            ...
            summary = f"[Earlier conversation ({len(older)} msgs): {' | '.join(summary_parts)}]"
...
class InfiniteContextManager:
    ...
    Disk persistence is via SQLite (SessionStore), synchronous writes.
    """

    def __init__(self, session_id: Optional[str] = None,
                 max_working_memory: int = 100,
                 max_episodic: int = 100000,
                 max_semantic: int = MAX_MEMORY_FACTS):
        ...
        self._store.ensure_session(self.session_id)
        self._load()

    async def add_message(self, role: str, content: str,
                         metadata: Optional[Dict] = None) -> None:
        """Add a message to the context. SQLite write is synchronous."""
    ...
    def _load(self) -> None:
        """Restore memory from SQLite session store."""
        try:
            rows = self._store.get_messages(self.session_id)
            self._working_memory = [
                {"role": r["role"], "content": r["content"], "timestamp": r["timestamp"]}
                for r in rows
            ]
            for r in rows:
                self._episodic.add_turn(
                    r["role"], r["content"],
                    turn_id=r.get("id"),
                    metadata=r.get("metadata", {}),
                )
            facts = self._store.load_semantic_facts(self.session_id)
            for key, fact in facts.items():
                if isinstance(fact, dict):
                    self._semantic.add_fact(
                        key, fact.get("content", ""),
                        importance=fact.get("importance", 1.0),
                    )
                elif isinstance(fact, str):
                    self._semantic.add_fact(key, fact)
        except Exception:
            self._working_memory = []
            self._episodic = EpisodicMemory(capacity=100000)
            self._semantic = SemanticMemory(max_facts=MAX_MEMORY_FACTS)
...
from tools import delegate_tool, skill_manager_tool, cronjob_tools, git_tools, reverse_tools, platform_dev_tools
...
class NexusOrchestrator:
    ...
    def __init__(
        ...
        logger.info(f"NexusOrchestrator v3.3.3 initialized: provider={self.ai_provider}, model={self.ai_model}, infinite_context=True")
    ...
    def llm_complete(self, system_prompt: str = "", user_prompt: str = "") -> str:
        """Synchronous single-turn LLM call — used as plan/work handler
        for multi-agent collaboration (CommanderCouncil).

        Makes a direct /chat/completions POST without tool loops or
        conversation tracking. Returns the model's text response.
        """
        return self._llm_call(self.ai_model, system_prompt, user_prompt)

    def llm_complete_with_model(self, model: str, system_prompt: str = "",
                                  user_prompt: str = "") -> str:
        """Like llm_complete but uses a specific model instead of the default."""
        return self._llm_call(model, system_prompt, user_prompt)

    def _llm_call(self, model: str, system_prompt: str = "",
                   user_prompt: str = "") -> str:
        """Low-level LLM call with a specific model."""
    ...
    def fetch_models(self) -> List[str]:
        ...
        try:
            ...
            if self.config.get("ai", {}).get("allow_self_signed", False):
                ctx.check_hostname = False
                ctx.verify_mode = ssl.CERT_NONE
            req = urllib.request.Request(
                f"{self.ai_base_url}/models",
                headers={"Accept": "application/json",
                         **( {"Authorization": f"Bearer {self.ai_api_key}"}
                            if self.ai_api_key else {})},
            )
    ...
    async def chat_stream(self, message: str, system_message: str = None,
        ...
        When company_mode=True, routes through CompanyOrchestrator.
        Otherwise loads conversation history from InfiniteContextManager.
        """
        if company_mode:
            async for event in self._chat_stream_company(message):
                yield event
            return

        ctx_mgr = await self._session_manager.get_session(session_id)
        history = await ctx_mgr.get_context(max_messages=50, include_semantic=True)

        messages = []
        if system_message:
            messages.append({"role": "system", "content": system_message})
        for m in history:
            if m.get("role") == "system" and system_message:
                continue
            messages.append(m)
        messages.append({"role": "user", "content": message})
    ...
    async def _chat_stream_company(self, message: str):
        """Route message through CompanyOrchestrator for Nexus mode."""
        try:
            from multiling.commander.council import CompanyOrchestrator
            co = CompanyOrchestrator(
                llm_complete=self.llm_complete,
                llm_with_model=self.llm_complete_with_model,
                fetch_models_fn=self.fetch_models,
            )
            yield {"type": "text", "content": f"\n[Nexus] Processing through multi-agent system...\n"}
            result = co.run(message)
            text = result.get("completion", "") or result.get("summary", "")
            if text:
                yield {"type": "text", "content": text}
            else:
                groups = result.get("groups", {})
                for gname, ginfo in groups.items():
                    yield {"type": "text", "content": f"\n  [{gname}] {ginfo.get('conclusion', 'done')}"}
            yield {"type": "done", "final_text": text or "Nexus task complete."}
        except Exception as e:
            yield {"type": "error", "message": f"Nexus error: {e}"}
            yield {"type": "done"}

    async def chat_stream_with_history(self, messages, tool_callback=None, reasoning_effort=None):
        """Stream with pre-built messages (bypasses memory system).
        Caller (Web UI) owns the conversation history and sends it
        verbatim.  The model sees the full conversation every time —
        no cold starts, no compression, no server-side state.
        """
        tools = get_tool_definitions(enabled_toolsets=self.enabled_toolsets)
        if not tools:
            yield {"type": "error", "message": "No tools available."}
            return
        async for event in self._stream_conversation(
            messages, tools, tool_callback=tool_callback, reasoning_effort=reasoning_effort
        ):
            yield event

    def _build_payload(self, live_messages, tools, reasoning_effort):
        """Build request payload and headers for upstream AI call."""
        payload = {
            "model": self.ai_model,
    ...
    async def _stream_conversation(self, messages, tools, session_id=None,
                                   tool_callback=None, max_iters=90, reasoning_effort=None):
        ...
        async def _persist_tool_result(
            ctx, live_messages, tc_entry, result_json, tool_call_id,
        ):
            """Add tool result to context and canonical message list.

            result_json is pre-serialized to avoid double json.dumps().
            """
        ...
        async def _close_round(ctx, live_messages, tool_calls_for_round, final_text):
            """Append assistant close marker to ctx and live_messages."""
            live_messages.append({
                "role": "assistant",
                "content": final_text,
            ...
            await ctx.add_message(
                "assistant",
                final_text,
                metadata={"tool_calls": tool_calls_for_round},
            )

        ctx = await self._get_context()
        if messages and messages[-1].get("role") == "user":
            await ctx.add_message(messages[-1]["role"], messages[-1]["content"])

        live_messages: List[Dict[str, Any]] = list(messages)

        api_call_count = 0

        connector = aiohttp.TCPConnector(limit=10, keepalive_timeout=TIMEOUT_AIOHTTP_KEEPALIVE)
        async with aiohttp.ClientSession(connector=connector) as session:
            while api_call_count < max_iters:
                ...
                payload, headers = self._build_payload(live_messages, tools, reasoning_effort)
                ...
                try:
                    async with session.post(
                        ...
                        json=payload,
                        headers=headers,
                        timeout=aiohttp.ClientTimeout(total=TIMEOUT_AIOHTTP_TOTAL),
                    ) as resp:
                        if resp.status != 200:
                            ...
                            if isinstance(result_obj, dict):
                                for k, v in result_obj.items():
                                    if isinstance(v, str) and len(v) > MAX_CONTENT:
                                        result_obj[k] = v[:MAX_CONTENT] + "…[truncated]"
                            tool_result_json = json.dumps(result_obj, ensure_ascii=False)
                            await _persist_tool_result(
                                ctx, live_messages, tc,
                                tool_result_json, tc.get("id", ""),
                            )
    ...
    def _run_with_upstream_ai(self, messages: List[Dict], tools: List[Dict],
                             session_id: Optional[str] = None, reasoning_effort: str = None) -> Dict:
    ...
    async def _async_run_conversation(
        ...
        while (
            ...
            if tool_calls:
                ...
                await ctx.add_message("assistant", content or "",
...
class TaskScheduler:
    ...
    async def stop(self):
        logger.info("TaskScheduler stopped (legacy mode)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="AgentHarness NexusOrchestrator v3.3.3 (Infinite Context)")
    parser.add_argument("--port", type=int, default=GATEWAY_DEFAULT_PORT, help="API server port")
