        ...
        if os.path.exists(disk_key):
            try:
                ...
                entry = CacheEntry(
                    ...
                    ttl=raw.get("ttl", 0), created=raw.get("created", 0),
                    accessed=raw.get("accessed", 0), hits=raw.get("hits", 0),
                    tags=raw.get("tags", []), priority=raw.get("priority", 0),
                )
                if not entry.is_expired():
                    self.memory.set(key, entry.value, ttl=entry.ttl, tags=entry.tags)
