...
try:
    ...
    class _CronIterFallback:
        """Small fallback for environments without croniter installed.

        Supports the common presets used in tests and degrades gracefully to
        a one-minute increment for unsupported expressions.
        """
        ...
        def __init__(self, expr, start_time):
            self.expr = expr.strip() if isinstance(expr, str) else str(expr)
            self.start_time = start_time

        def get_next(self, _cls):
            delta = self._PRESET_SECONDS.get(self.expr, 60)
            return self.start_time + datetime.timedelta(seconds=delta)

    class croniter:  # type: ignore
        @staticmethod
        def croniter(expr, start_time):
            return _CronIterFallback(expr, start_time)
