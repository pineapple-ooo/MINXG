...
Override per-deployment via env var ``AgentHarness_TOOL_CAP``.
...
def _is_termux_host() -> bool:
    """Best-effort Termux / ZeroTermux detection.

    We keep this local so higher-level CLI probes can reuse the same
    platform classification without duplicating env checks.
    """
...
def _platform_key() -> str:
    ...
    # `platform.system()` is often just "Linux" in Termux, so we need
    # an explicit runtime probe rather than relying on the OS name.
    if sysname == "android" or (sysname.startswith("linux") and _is_termux_host()):
...
def cap_for(platform_key: Optional[str] = None) -> int:
    ...
    Honours ``AgentHarness_TOOL_CAP`` env var when set (deployment override).
    """
    env_override = os.environ.get("AgentHarness_TOOL_CAP")
