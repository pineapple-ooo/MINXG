...
def test_env_override_is_honoured(monkeypatch):
    monkeypatch.setenv("AgentHarness_TOOL_CAP", "42")
    cap = pc.cap_for()
    assert cap == 42
...
def test_invalidate_drops_cache(monkeypatch):
    ...
    monkeypatch.setenv("AgentHarness_TOOL_CAP", "3")
    pc.invalidate()
    a = pc.active_tools()
    assert len(a) <= 3


class TestPlatformEnvironmentDetection:
    def test_termux_like_linux_is_classified_as_android(self, monkeypatch):
        monkeypatch.setenv("TERMUX_VERSION", "1")
        monkeypatch.delenv("ZERO_TERMUX", raising=False)
        monkeypatch.delenv("ZEROTERMUX_VERSION", raising=False)
        monkeypatch.delenv("PREFIX", raising=False)
        monkeypatch.setattr(pc._plat, "system", lambda: "Linux")
        pc.invalidate()
        try:
            assert pc.detect_platform_key() == "android"
        finally:
            pc.invalidate()

    def test_doctor_runtime_label_helper(self):
        from multiligua_cli import doctor
        assert doctor._runtime_label("android", "Linux") == "Termux / Android"
        assert doctor._runtime_label("linux", "Linux") == "POSIX"
