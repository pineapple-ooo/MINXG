...
Tests cover:
  - AgentHarnessMenu can be constructed with title + 2 options
...
class TestAgentHarnessMenuConstruction:
    def test_basic_construction(self):
        menu = wizard_mod.AgentHarnessMenu("Pick one", ["alpha", "beta"])
    ...
    def test_empty_descriptions_defaults_to_blanks(self):
        menu = wizard_mod.AgentHarnessMenu("Title", ["a", "b", "c"])
        assert menu.descriptions == ["", "", ""]

    def test_descriptions_preserved(self):
        menu = wizard_mod.AgentHarnessMenu("Title", ["a", "b"], ["desc a", "desc b"])
        assert menu.descriptions == ["desc a", "desc b"]


# ── run() with readchar unavailable ────────────────────────────────────────

class TestAgentHarnessMenuRunNoReadchar:
    """When readchar is missing, AgentHarnessMenu falls back to numbered input."""

    def test_run_returns_index_for_valid_input(self, monkeypatch, capsys):
        monkeypatch.setattr(wizard_mod, "HAS_READCHAR", False)
        menu = wizard_mod.AgentHarnessMenu("Title", ["opt_a", "opt_b", "opt_c"])
    ...
    def test_run_returns_none_on_cancel_q(self, monkeypatch):
        monkeypatch.setattr(wizard_mod, "HAS_READCHAR", False)
        menu = wizard_mod.AgentHarnessMenu("Title", ["opt_a", "opt_b"])
    ...
    def test_run_returns_none_on_eof(self, monkeypatch):
        monkeypatch.setattr(wizard_mod, "HAS_READCHAR", False)
        menu = wizard_mod.AgentHarnessMenu("Title", ["opt_a"])
...
class TestAgentHarnessMenuRunWithReadchar:
    """When readchar is present, AgentHarnessMenu uses arrow-key navigation."""
    ...
    def test_run_returns_selected_on_enter(self, monkeypatch):
        ...
        menu = wizard_mod.AgentHarnessMenu("Title", ["alpha", "beta", "gamma"])
        menu.selected = 0
    ...
    def test_run_returns_none_on_q(self, monkeypatch):
        monkeypatch.setattr(wizard_mod, "HAS_READCHAR", True)
        menu = wizard_mod.AgentHarnessMenu("Title", ["alpha"])
        fake_keys = iter(["q"])
        with mock.patch.object(wizard_mod.readchar, "readkey", side_effect=lambda: next(fake_keys)):
            result = menu.run()
        assert result is None
...
class TestDescriptionRendering:
    def test_descriptions_render_alongside_options(self, monkeypatch, capsys):
        ...
        # Direct call to print_option_item (used inside AgentHarnessMenu)
