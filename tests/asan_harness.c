...
int main(void) {
    /* arena */
    agent_harness_arena_t a = agent_harness_arena_create(8192);
    assert(a);
    for (int i = 0; i < 50; i++) {
        void* p1 = agent_harness_arena_alloc(a, 80);
        void* p2 = agent_harness_arena_alloc(a, 200);
        assert(p1); assert(p2);
        memset(p1, 0xaa, 80);
        memset(p2, 0xbb, 200);
        agent_harness_arena_reset(a);
    }
    agent_harness_arena_destroy(a);

    /* slab */
    agent_harness_slab_t s = agent_harness_slab_create(40, 16);
    assert(s);
    void* items[8];
    for (int i = 0; i < 8; i++) {
        items[i] = agent_harness_slab_alloc(s);
        assert(items[i]);
        memset(items[i], 0xcd, 40);
    }
    for (int i = 0; i < 8; i++) agent_harness_slab_free(s, items[i]);
    /* realloc/free cycle 5 times */
    for (int cycle = 0; cycle < 5; cycle++) {
        void* t[20];
        for (int i = 0; i < 20; i++) t[i] = agent_harness_slab_alloc(s);
        assert(t[0]);
        for (int i = 0; i < 20; i++) agent_harness_slab_free(s, t[i]);
    }
    agent_harness_slab_destroy(s);

    /* ring buffer */
    agent_harness_rb_t rb = agent_harness_rb_create(8, 4);
    assert(rb);
    unsigned char in[8]  = {1,2,3,4,5,6,7,8};
    unsigned char out[8] = {0};
    int rc;
    rc = agent_harness_rb_push(rb, in);  assert(rc == 0);
    rc = agent_harness_rb_pop(rb, out);  assert(rc == 0);
    /* fill to capacity, expect overflow */
    for (int i = 0; i < 20; i++) {
        rc = agent_harness_rb_push(rb, in);
        if (rc == 0) {
            rc = agent_harness_rb_pop(rb, out);
            assert(rc == 0);
        } else {
    ...
    rc = agent_harness_statistics(vals, 16, &mean, &std, &med, &mn, &mx, &sum);
    assert(rc == 0);
    assert(mean == 8.5);
    assert(sum == 136.0);
    ...
    double v = agent_harness_evolve_ncd_v1(a5, 5, b5, 5);
    printf("NCD pair = %f\n", v);
    assert(v >= 0.0 && v <= 1.0);
    ...
    double matrix[9] = {0};
    rc = agent_harness_evolve_ncd_matrix_v1(seqs, lengths, 3, matrix);
    assert(rc == 0);
