...
**名字**：`agent_harness-mcp` 或 `mcp-workers`
...
**实施**：
- GitHub仓库名改为 `agent_harness-mcp`
- PyPI包名保持 `agent_harness-beta`（已有下载量）
- README标题：`AgentHarness MCP — Connect Claude to 70+ AI Workers`
...
4. 不需要完全抛弃AgentHarness品牌

## 实施步骤

1. **GitHub仓库重命名**
   ```
   AgentHarness-Beta → agent_harness-mcp
   ```
...
3. **添加MCP Server**（已完成）
   - `agent_harness/mcp_server.py`

4. **创建Claude Code Skills**（已完成）
   - `.claude/skills/agent_harness/`

5. **发布到Product Hunt + Hacker News**
   - 标题：`AgentHarness MCP — Connect Claude Code to 70+ AI Workers`
