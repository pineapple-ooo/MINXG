# AgentHarness MCP Server

[![PyPI version](https://img.shields.io/pypi/v/agent_harness-beta.svg)](https://pypi.org/project/agent_harness-beta/)
...
AgentHarness is a modular AI worker platform. This repo exposes all workers as **MCP tools**, so any MCP-compatible AI client (Claude Code, Cursor, Windsurf, VS Code Copilot, ChatGPT) can call them directly.
...
```bash
claude mcp add agent_harness -s user -- python -m agent_harness.mcp_server
```
...
```bash
# List all tools
python -m agent_harness.mcp_server list

# Call a tool directly
python -m agent_harness.mcp_server call fs_io read_file --path /etc/hosts
...
| Feature | AgentHarness | Other MCP Servers |
|---------|-------|-------------------|
...
→ AgentHarness reads the file, hashes it, returns the digest.
...
→ AgentHarness runs ping + nslookup, returns latency and IP.
...
→ AgentHarness's math worker computes and returns results.
...
→ AgentHarness searches, reads, aggregates — all through MCP tools.
...
[![Star History Chart](https://api.star-history.com/svg?repos=pineapple-ooo/AgentHarness-Beta&type=Date)](https://star-history.com/#pineapple-ooo/AgentHarness-Beta&Date)

---

**Built with ❤️ by the AgentHarness team.**

Questions? [Open an issue](https://github.com/pineapple-ooo/AgentHarness-Beta/issues).
