                ...
                try:
                    for pid_dir in Path("/proc").iterdir():
                        ...
                        try:
                            ...
                            # /proc entry vanished or unreadable — skip silently
                            pass
                except Exception:
                    pass
...
SYSTEM_SCHEMA = {
    ...
    "properties": {
        "action": {
            "type": "string",
            "enum": ["info", "disk", "network", "process"],
            "description": "System info, disk usage, network info, or process list",
        },
    },
    "required": ["action"],
}
...
def _handle_system(args: dict) -> str:
    ...
    if action == "info":
        return _handle_system_info(args)
    elif action == "disk":
        return _handle_disk_usage(args)
    elif action == "network":
        return _handle_network_info(args)
    elif action == "process":
        return _handle_process_list(args)
