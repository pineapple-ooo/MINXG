...
from agent_harness.core_ops.file_safety import (
    ...
    is_write_denied as _is_write_denied,
)
...
def _handle_write_file(args: dict) -> str:
    ...
    denied, reason = _is_write_denied(path)
    if denied:
        return json.dumps({"error": f"Cannot write to protected path: {reason}"})
...
FILE_SCHEMA = {
    ...
    "properties": {
        "action": {
            "type": "string",
            "enum": ["read", "write", "search", "patch", "ls", "info"],
            "description": "File action: read, write, search, patch, list directory, or file info",
        },
        ...
        "file_glob": {"type": "string", "description": "File glob filter (for search)"},
        "offset": {"type": "integer", "description": "Read offset (for read)"},
...
def _handle_file(args: dict) -> str:
    action = args.get("action", "")
    if action == "read":
        return _handle_read_file(args)
    elif action == "write":
        return _handle_write_file(args)
    elif action == "search":
        return _handle_search_files(args)
    elif action == "patch":
        return _handle_patch(args)
    elif action == "ls":
        return _handle_ls_dir(args)
    elif action == "info":
        return _handle_file_info(args)
