...
DELEGATE_SCHEMA = {
    ...
    "properties": {
        "mode": {
            "type": "string",
            "enum": ["single", "batch"],
            "description": "Single task or batch of parallel tasks",
        },
        "goal": {"type": "string", "description": "Task goal (mode=single)"},
        ...
        "tasks": {
            ...
            "description": "Array of task objects (mode=batch), max 5",
        },
...
def _handle_delegate(args: dict) -> str:
    mode = args.get("mode", "")
    if mode == "batch":
        return _handle_delegate_batch(args)
    return _handle_delegate_task(args)
