...
def _handle_execute_javascript(args: dict) -> str:
    ...
    try:
        ...
        return json.dumps({"error": "Node.js not found. Install with: pkg install nodejs"})
    except subprocess.TimeoutExpired:
        return json.dumps({"error": f"JavaScript execution timed out after {timeout}s"})
    except Exception as e:
        return json.dumps({"error": f"JS execution error: {e}"})


def _handle_execute_rust(args: dict) -> str:
    ...
    try:
        ...
        compile_result = subprocess.run(
            ["rustc", src, "-o", bin_name],
            capture_output=True, text=True, timeout=timeout,
        )
        if compile_result.returncode != 0:
            return json.dumps({
                "ok": False, "error": "Compilation failed",
                "stderr": compile_result.stderr[:5000],
            })
        run_result = subprocess.run(
            [bin_name], capture_output=True, text=True, timeout=timeout,
        )
        os.unlink(src)
        try:
            os.unlink(bin_name)
        ...
        return json.dumps({
            ...
            "stderr": run_result.stderr[:5000] if run_result.stderr else "",
            "exit_code": run_result.returncode,
        })
    except FileNotFoundError:
        return json.dumps({"error": "Rust not found. Install with: pkg install rust"})
    except subprocess.TimeoutExpired:
        return json.dumps({"error": f"Rust execution timed out after {timeout}s"})
    except Exception as e:
        return json.dumps({"error": f"Rust execution error: {e}"})
...
EXECUTE_SCHEMA = {
    ...
    "properties": {
        "mode": {
            "type": "string",
            "enum": ["command", "code"],
            "description": "Mode: 'command' for shell, 'code' for inline code execution",
        },
        "language": {
            "type": "string",
            "enum": ["python", "shell", "javascript", "node", "rust"],
            "description": "Language for code execution (mode=code). python/shell/js/rust",
            "default": "python",
        },
        "command": {"type": "string", "description": "Shell command (mode=command)"},
        "code": {"type": "string", "description": "Code to execute (mode=code)"},
...
def _handle_execute(args: dict) -> str:
    mode = args.get("mode", "")
    if mode == "command":
        return _handle_execute_command(args)
    elif mode == "code":
        lang = args.get("language", "python") or "python"
        if lang in ("javascript", "js", "node"):
            return _handle_execute_javascript(args)
        elif lang == "rust":
            return _handle_execute_rust(args)
        return _handle_execute_code(args)
    return json.dumps({"error": f"Unknown execute mode: {mode}"})
