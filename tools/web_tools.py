...
WEB_SCHEMA = {
    ...
    "properties": {
        "action": {
            "type": "string",
            "enum": ["fetch", "status", "ping"],
            "description": "Action: fetch URL content, check HTTP status, or ping host",
        },
        "url": {"type": "string", "description": "URL for fetch/status"},
        "host": {"type": "string", "description": "Host for ping"},
        "method": {"type": "string", "description": "HTTP method (fetch)", "default": "GET"},
        "headers": {"type": "object", "description": "Custom headers (fetch)"},
        "body": {"type": "string", "description": "Request body (fetch POST/PUT)"},
...
def _handle_web(args: dict) -> str:
    action = args.get("action", "")
    if action == "fetch":
        return _handle_fetch_url(args)
    elif action == "status":
        return _handle_http_status(args)
    elif action == "ping":
        return _handle_ping_tool(args)
    return json.dumps({"error": f"Unknown web action: {action}"})
