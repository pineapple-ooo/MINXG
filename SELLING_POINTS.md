# AgentHarness — Capability Summary (v0.18.5)

This document lists what AgentHarness actually ships with, measured from the codebase.
No hyperbole, no competitor bashing — just facts you can verify with `agent_harness doctor`
and `agent_harness tools`.
...
- **59 worker classes** exported from `agent_harness.__init__`
...
|---------|-------------------|
| **Chat CLI** (`agent_harness`) | Streaming chat, tool calls, 32+ providers, `/model` `/lang` `/help` |
| **API Gateway** (`agent_harness gateway`) | OpenAI-compatible `/v1/chat/completions`, cost tracking, rate limits |
| **MCP Server** (`agent_harness gateway --mcp`) | All 59 workers exposed as MCP tools for Claude Code / Cursor / Windsurf |
| **Skill System** (`agent_harness skill …`) | Markdown instruction packs, local/remote catalogs, no code execution on install |
...
|--------|--------|-------------------|
| Geometric Algebra | `agent_harness.ga` | ~47 |
| Category Theory | `agent_harness.cat` | ~79 |
| Information Geometry | `agent_harness.infogeo` | ~51 |
| Topology | `agent_harness.topo` | ~53 |
| Chaos Theory | `agent_harness.chaos` | ~23 |
| Fiber Bundles | `agent_harness.fiber` | ~53 |
| Symbolic Diff Algebra | `agent_harness.symbdiff` | ~20 |
| **GeometryWorker** | `agent_harness.five_pillars.math_pillar.geometry` | driver integration |
...
| Rust | `rust_core/` | `agent_harness_rust_core` crate via ctypes |
| Go | `go_core/` | JSON-RPC over stdio |
| Java | `java_core/` | JVM daemon + vector engine |
| Julia | `agent_harness/five_pillars/polyglot/julia_worker.py` | `agent_harness.contracts.runtime.julia` |
| R | `agent_harness/five_pillars/polyglot/r_worker.py` + `r_scripts/` | R subprocess |
| WASM | `agent_harness/five_pillars/polyglot/wasm_worker.py` | Wasmtime |
| Datalog | `agent_harness/five_pillars/polyglot/datalog_worker.py` | Clingo / pyDatalog |
...
| Web UI | Experimental | `agent_harness web` — browser chat |
...
| Multi-Agent Coding | Extension | `agent_harness ext enable agent_harness-multiagent` → Planner/Coder/Reviewer loop |
...
```
agent_harness                    # chat CLI
agent_harness setup              # wizard
agent_harness config / status    # inspection
agent_harness tools / model / api / key / lang
agent_harness gateway [--detach] [--mcp]   # API / MCP
agent_harness doctor             # self-check
agent_harness ext / skill / features / themes / export / import / bench / replay / update / help
```
