...
def test_crypto():
    from agent_harness.five_pillars.scalar.core_native import sha256, sha512, hmac_sha256, hex_encode, hex_decode
...
def test_encoding():
    from agent_harness.five_pillars.scalar.core_native import base64_encode, base64_decode
...
def test_text():
    from agent_harness.five_pillars.scalar.core_native import is_valid_utf8, trim, tokenize
...
def test_word_freq():
    from agent_harness.five_pillars.scalar.core_native import word_frequency
...
if __name__ == "__main__":
    print("=== AgentHarness Python Bridge Tests ===\n")
    try:
        test_import()
        test_crypto()
        test_encoding()
