# AgentHarness vs Hermes Agent — Benchmark (v0.18.2 vs Hermes 2.3.x)

*Both MIT-licensed. AgentHarness builds on prior art; this is engineering, not marketing.*

---

## TL;DR

| Dimension | Hermes Agent | AgentHarness | Winner |
|-----------|-------------|-------|--------|
| Languages in core | 1 (Python) | **7** (Rust/C++/R/Julia/Datalog/Wasm/Go) | **AgentHarness** |
| Gateway platforms | 20 | **19 + own gateway = 20** | Tie |
| Tool files | 96 | **743 `@tool` methods** (facade-collapsed to ~200 visible) | **AgentHarness** |
| Tool chain (1 LLM call → N tools) | ❌ per-call | **✓ ToolChainExecutor** | **AgentHarness** |
| Rust math (FFT/Kalman/Lyapunov) | ❌ | **✓ sub-ms** | **AgentHarness** |
| Lock-free C++ ringbuf | ❌ | **✓ SPSC zero-copy** | **AgentHarness** |
| Self-evolution loop | ❌ | **✓ TwinEngine + FieldForge** | **AgentHarness** |
| Math pillars | 0 | **6** (GA/cat/infogeo/topo/chaos/fiber) | **AgentHarness** |
| Tests | — | **713 passed** | **AgentHarness** |
| Architecture | Python monolith | 3-tier + 7 pillars + polyglot adapters | Style |

> Hermes is a solid generalist. AgentHarness is a specialist that happens to also be a generalist.
...
**AgentHarness:** Seven languages, each doing what it does best:
...
**AgentHarness:** 19 channels via `unified-channel` adapter + its own gateway.
Same surface, but every message passes through AgentHarness's tool chain before delivery.
...
AgentHarness's own gateway is OpenAI-compatible. Swap providers without touching tools.
...
**AgentHarness:** 743 `@tool` method decorators across 56 workers.
...
**AgentHarness:** One LLM call → plan (JSON array of steps) → ToolChainExecutor
...
**AgentHarness:** Six mathematical pillars, each owning a stable operator ID range:
...
## 6. Algorithms (what AgentHarness has that Hermes doesn't)
...
**AgentHarness:** The **DriverEngine watches itself fail** and auto-replaces operators:
...
**AgentHarness:** Three-tier architecture with pillar isolation:
...
AgentHarness is younger. These are honest gaps.
...
- If you want **raw polyglot performance, math muscle, and tool chains that don't burn LLM calls**: use AgentHarness

AgentHarness isn't trying to be Hermes. It's trying to be the version of Hermes that
