#!/usr/bin/env bash
# AgentHarness Rust build — Termux-friendly fallback.
...
if cd "$RUST" && cargo build --release 2>&1; then
    cp "$RUST/target/release/libagent_harness_rust.so" "$OUT/" 2>/dev/null || true
    cp "$RUST/target/release/libagent_harness_rust.dylib" "$OUT/" 2>/dev/null || true
    echo "Cargo build OK"
    ls -la "$OUT"
    exit 0
fi
...
rustc --edition 2021 \
    --crate-type cdylib \
    --crate-name agent_harness_rust \
    -o "$OUT/libagent_harness_rust.so" \
    -L /data/data/com.termux/files/usr/lib \
    "${SOURCES[@]}" \
    --cfg 'feature=""' \
    2>&1 | tail -20

if [ -f "$OUT/libagent_harness_rust.so" ]; then
