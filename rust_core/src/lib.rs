//! agent_harness_rust_core -- Rust performance core for AgentHarness.
//!
...
#[no_mangle]
pub extern "C" fn agent_harness_distance_ffi(a: *const f64, b: *const f64, len: usize) -> f64 {
...
#[no_mangle]
pub extern "C" fn agent_harness_polygon_area_ffi(points_flat: *const f64, count: usize) -> f64 {
...
/// Return the AgentHarness Rust core version string.
/// Caller must free the returned string with rust_core_free.
#[no_mangle]
pub extern "C" fn rust_core_version() -> *mut std::os::raw::c_char {
    let v = std::ffi::CString::new("0.18.6").unwrap();
    v.into_raw()
}

/// Free a string previously returned by a Rust FFI function.
#[no_mangle]
pub extern "C" fn rust_core_free(s: *mut std::os::raw::c_char) {
    if !s.is_null() {
        unsafe { let _ = std::ffi::CString::from_raw(s); }
    }
}

/// Return the number of math operators available in the Rust core.
#[no_mangle]
pub extern "C" fn rust_math_operator_count() -> i32 {
    2 // distance, polygon_area
}

/// Logistic map iteration: x_{n+1} = r * x_n * (1 - x_n).
/// Returns the last value after `steps` iterations.
#[no_mangle]
pub extern "C" fn rust_logistic_map(r: f64, x0: f64, steps: i32) -> f64 {
    let mut x = x0;
...
/// Lorenz system integration via Euler method for `steps` iterations.
/// Writes [x, y, z] to the output buffer provided by the caller.
#[no_mangle]
pub extern "C" fn rust_lorenz(
    x0: f64,
    y0: f64,
    z0: f64,
    sigma: f64,
