...
_PROJ_CANDIDATES = (
    Path(os.environ.get("AgentHarness_NATIVE_ROOT", "")),
    Path(__file__).resolve().parent,
    Path("/storage/emulated/0/AgentHarness"),
    Path("/storage/emulated/0/AgentHarness-Beta-0.11.0"),
)
...
class GoCore:
    ...
    def _setup_funcs(cls):
        lib = cls._lib
        lib.AgentHarnessGoVersion.argtypes = []
        lib.AgentHarnessGoVersion.restype = ctypes.c_char_p
        lib.AgentHarnessHealthCheck.argtypes = []
        lib.AgentHarnessHealthCheck.restype = ctypes.c_int
        lib.AgentHarnessTextSearchBMH.argtypes = [ctypes.c_char_p, ctypes.c_size_t,
                                            ctypes.c_char_p, ctypes.c_size_t]
        lib.AgentHarnessTextSearchBMH.restype = ctypes.c_longlong
        lib.AgentHarnessTextSearchBMHReverse.argtypes = lib.AgentHarnessTextSearchBMH.argtypes
        lib.AgentHarnessTextSearchBMHReverse.restype = ctypes.c_longlong
        lib.AgentHarnessTextCount.argtypes = lib.AgentHarnessTextSearchBMH.argtypes
        lib.AgentHarnessTextCount.restype = ctypes.c_int
        lib.AgentHarnessStrLower.argtypes = [ctypes.c_char_p, ctypes.c_size_t]
        lib.AgentHarnessStrLower.restype = ctypes.c_size_t
        lib.AgentHarnessStrUpper.argtypes = lib.AgentHarnessStrLower.argtypes
        lib.AgentHarnessStrUpper.restype = ctypes.c_size_t
        lib.AgentHarnessStrTrim.argtypes = lib.AgentHarnessStrLower.argtypes
        lib.AgentHarnessStrTrim.restype = ctypes.c_size_t
        lib.AgentHarnessGlobMatch.argtypes = [ctypes.c_char_p, ctypes.c_char_p]
        lib.AgentHarnessGlobMatch.restype = ctypes.c_int
        lib.AgentHarnessGlobMatchCI.argtypes = lib.AgentHarnessGlobMatch.argtypes
        lib.AgentHarnessGlobMatchCI.restype = ctypes.c_int
        lib.AgentHarnessUtf8Valid.argtypes = [ctypes.c_char_p, ctypes.c_size_t]
        lib.AgentHarnessUtf8Valid.restype = ctypes.c_int
        lib.AgentHarnessUtf8Codepoints.argtypes = lib.AgentHarnessUtf8Valid.argtypes
        lib.AgentHarnessUtf8Codepoints.restype = ctypes.c_int
        lib.AgentHarnessSlugify.argtypes = [ctypes.c_char_p, ctypes.c_size_t,
                                      ctypes.c_char_p, ctypes.c_size_t]
        lib.AgentHarnessSlugify.restype = ctypes.c_size_t
        lib.AgentHarnessTruncate.argtypes = [ctypes.c_char_p, ctypes.c_size_t,
                                       ctypes.c_size_t, ctypes.c_char_p,
                                       ctypes.c_size_t, ctypes.c_char_p,
                                       ctypes.c_size_t]
        lib.AgentHarnessTruncate.restype = ctypes.c_size_t
        lib.AgentHarnessWordFreqHash.argtypes = [ctypes.c_char_p, ctypes.c_size_t,
                                          ctypes.c_int, ctypes.c_char_p, ctypes.c_size_t]
        lib.AgentHarnessWordFreqHash.restype = ctypes.c_size_t
        lib.AgentHarnessNormalizeWS.argtypes = [ctypes.c_char_p, ctypes.c_size_t,
                                          ctypes.c_int, ctypes.c_char_p, ctypes.c_size_t]
        lib.AgentHarnessNormalizeWS.restype = ctypes.c_size_t
        lib.AgentHarnessBaseConvert.argtypes = [ctypes.c_char_p, ctypes.c_int,
                                         ctypes.c_int, ctypes.c_char_p, ctypes.c_size_t]
        lib.AgentHarnessBaseConvert.restype = ctypes.c_int
        lib.AgentHarnessExtractURLs.argtypes = [ctypes.c_char_p, ctypes.c_size_t,
                                          ctypes.c_char_p, ctypes.c_size_t, ctypes.c_int]
        lib.AgentHarnessExtractURLs.restype = ctypes.c_int
        lib.AgentHarnessExtractEmails.argtypes = lib.AgentHarnessExtractURLs.argtypes
        lib.AgentHarnessExtractEmails.restype = ctypes.c_int
        lib.AgentHarnessExtractHashtags.argtypes = lib.AgentHarnessExtractURLs.argtypes
        lib.AgentHarnessExtractHashtags.restype = ctypes.c_int
        lib.AgentHarnessFree.argtypes = [ctypes.c_void_p]
        lib.AgentHarnessFree.restype = None
    ...
    def version(self) -> str:
        ...
        ptr = self._lib.AgentHarnessGoVersion()
    ...
    def health_check(self) -> bool:
        ...
        return bool(self._lib.AgentHarnessHealthCheck())

    def bmh_search(self, haystack: str, needle: str) -> int:
        if not self._lib: return -1
        hb = haystack.encode('utf-8'); nb = needle.encode('utf-8')
        pos = self._lib.AgentHarnessTextSearchBMH(hb, len(hb), nb, len(nb))
        return int(pos)

    def bmh_count(self, haystack: str, needle: str) -> int:
        if not self._lib: return haystack.count(needle)
        hb = haystack.encode('utf-8'); nb = needle.encode('utf-8')
        return int(self._lib.AgentHarnessTextCount(hb, len(hb), nb, len(nb)))
...
class ShellCore:
    ...
    def now(format: str = "%Y-%m-%d %H:%M:%S") -> str:
        """Return current time formatted by Python strftime (no shell)."""
        return time.strftime(format)

    @staticmethod
    def seq(start: int, end: int, step: int = 1) -> List[int]:
        """Return range(start, end+1, step) — pure Python, no shell."""
...
if __name__ == "__main__":
    print("=== AgentHarness Native Bridge Status ===")
    s = status()
    for k, v in s.items():
        print(f"  {k}: {v}")
