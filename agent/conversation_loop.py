...
class ConversationLoop:
    ...
    def __init__(
        ...
        # Dedup cache for tool calls within a single turn: (tool_name, stable_args_key) -> result
    ...
    def get_activity_summary(self) -> dict:
        ...
        return {
            ...
            "tool_cache_size": len(self._tool_call_cache),
        }

    @staticmethod
    def _deduplicate_tool_calls(tool_calls: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Remove duplicate (tool_name, arguments) pairs within a single model response.

        Hermes upstream pattern: collapse repeated identical tool calls at the
        model-output layer so the executor never sees them.
        """
        seen: set = set()
        ...
        for tc in tool_calls:
            ...
            if key not in seen:
                ...
                logger.debug("Removed duplicate tool call: %s", fn.get("name"))
        return unique if len(unique) < len(tool_calls) else tool_calls

    @staticmethod
    def _stable_args_key(args: Dict[str, Any]) -> str:
        """Create a stable cache key from tool arguments.

        Uses JSON serialization with sorted keys for deterministic ordering.
        Skips None values to avoid cache misses from optional params.
        """
        try:
            normalized = {k: v for k, v in args.items() if v is not None}
            return json.dumps(normalized, sort_keys=True, ensure_ascii=False, default=str)
        except Exception:
            return str(args)

    def _get_cached_tool_result(self, tool_name: str, args: Dict[str, Any]) -> Optional[Any]:
        """Return cached result if this exact tool call was already made this turn."""
        key = f"{tool_name}:{self._stable_args_key(args)}"
        return self._tool_call_cache.get(key)

    def _cache_tool_result(self, tool_name: str, args: Dict[str, Any], result: Any) -> None:
        """Cache a tool result for dedup within this turn."""
        key = f"{tool_name}:{self._stable_args_key(args)}"
        self._tool_call_cache[key] = result
    ...
    def run_turn(
        ...
        # Per-turn caches; cleared on each new turn so we do not leak state.
        ...
        while (
            ...
            try:
                ...
                if response.get("tool_calls"):
                    response["tool_calls"] = self._deduplicate_tool_calls(response["tool_calls"])
                    assistant_message = {
                        "role": "assistant",
                    ...
                    for tool_call in response["tool_calls"]:
                        ...
                        # Dedup: if this exact tool call was already executed this turn, reuse it.
                        cached = self._get_cached_tool_result(fn_name, fn_args)
                        if cached is not None:
                            result = cached
                        else:
                            result = execute_tool_fn(fn_name, fn_args)
                            self._cache_tool_result(fn_name, fn_args, result)
