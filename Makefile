# AgentHarness Polyglot Build System
# =============================
#
# Language responsibilities:
#   C    → libagent_harness_c.so    (core data structures, text engine, memory pools)
#   C++  → libagent_harness_cpp.a   (crypto, json_fast, compress, RAII wrappers)
#   Go   → agent_harness-gateway    (HTTP/WS server, rate limiter, health checker)
...
go-build: $(C_TARGET)
	...
	cd $(ROOT)/go_core && CGO_ENABLED=1 \
		CGO_LDFLAGS="-L$(BUILD_DIR) -lagent_harness_c -lssl -lcrypto -lpthread" \
...
"
	...
	@pkill -f "agent_harness-gateway --port 19004" 2>/dev/null || true
	@echo ""
