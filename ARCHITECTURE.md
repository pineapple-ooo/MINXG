...
(`agent_harness/`, `multiling/`, `multiligua_cli/`) and nothing previously
...
multiling/         AI orchestration       "the engine"
        │
        ├── multiling/commander/          hierarchical multi-agent orchestration
        │       CommanderAI, TaskBoard, AgentPool, ConflictGuard,
        │       LoopGuardian (anti-loop), Reviewer
        │
tools/             chat-agent tools       "what the agent can call"
        │
agent_harness/             math + MCP workers     "the science + the MCP surface"
```

- **`multiligua_cli/`** — everything a user actually runs: `main.py`
  (argparse entry point + every `agent_harness <command>` dispatch), the TUI
  chat, `features.py`, `skill_cli.py`, `gateway_cli.py`, themes,
  memory viz, cost tracking. If you're adding a new `agent_harness <verb>`
...
- **`multiling/`** — the runtime underneath the CLI: `orchestrator.py`
  ...
  metadata describing the `tools/` registry for the CLI's `agent_harness tools`
...
- **`tools/`** — the function-calling surface the chat agent actually
  ...
  into `tools/registry.py` at import time. `agent_harness doctor`'s "active
  tools" count is this registry.
- **`agent_harness/`** — two things share this package:
  1. `agent_harness/five_pillars/` + friends (`ga`, `cat`, `infogeo`, `fiber`,
  ...
  2. `agent_harness/workers/` + `agent_harness/mcp_server.py` — a **second**,
     ...
     `agent_harness/workers/`, AgentHarness's own chat loop talks to `tools/`.
  3. `agent_harness/core_ops/` — shared logic used by *both* of the above where
     they'd otherwise duplicate something safety-relevant (e.g.
     `file_safety.py`'s blocked-path/binary-file checks, used by both
     `tools/file_tools.py` and `agent_harness/workers/file/file_workers.py`;
...
- **`gateway/`** — the OpenAI-compatible HTTP API (`gateway/server.py`)
  ...
  Runs standalone via `agent_harness gateway`.
- **`extensions/`** — user-installable *executable* plugins (as opposed
  ...
  `agent_harness ext ...`. Built-ins live in `extensions/builtin/`.
- **`skills/`** — markdown instruction bundles AgentHarness ships with, in
  `<category>/<name>/SKILL.md` layout. `agent_harness skill ...` (backed by
  `agent_harness/core_ops/skill_registry.py`) can install more from a local
...
- **`c_core/`, `cpp_core/`, `rust_core/`, `go_core/`, `java_core/`,
  `ts_core/`** — polyglot bridges for the math subsystem. `agent_harness
...
`tools/` vs `agent_harness/workers/`. They're not accidental duplication — they
...
making sure the actual logic underneath is shared (`agent_harness/core_ops/`),
