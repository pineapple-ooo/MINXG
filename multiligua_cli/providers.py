"""
multiligua_cli/providers.py — AgentHarness AI provider registry (v0.18.3).
...
# Alias names that downstream code still imports.  These are populated by
# the ProviderRegistry class below so legacy callers see the same shape.
...
class ProviderRegistry:
    """Typed provider registry with back-compat dict view."""

    def __init__(self):
        self._by_slug: Dict[str, Dict] = {}
        _EMOJI = {
            "openai": "○", "anthropic": "▲", "google": "◆", "deepseek": "◉",
            "openrouter": "◇", "nvidia": "▣", "xai": "✕", "minimax": "✦",
            "alibaba": "●", "moonshot": "◌", "zhipu": "◐", "mistral": "◉",
            "ollama": "λ", "llamacpp": "#", "vllm": "▤", "custom": "⋯",
        }
        for p in PROVIDER_REGISTRY:
            slug = p["slug"]
            models = MODEL_CATALOGUE.get(slug, [])
            default_model = models[0]["id"] if models else ""
            self._by_slug[slug] = {
                "slug": slug,
                "name": p.get("display") or slug,
                "display": p.get("display") or slug,
                "base_url": p.get("base_url") or "",
                "default_url": p.get("base_url") or "",
                ...
                "emoji": _EMOJI.get(slug, "◆"),
                "description": p.get("description", p.get("display", slug)),
                "needs_api_key": bool(p.get("env_key")),
            }
        for k, v in _os_mod.environ.items():
            if k.startswith("AgentHarness_BASE_URL_"):
                slug = k[len("AgentHarness_BASE_URL_"):].lower()
                if slug in self._by_slug:
                    self._by_slug[slug]["base_url"] = v
                    self._by_slug[slug]["default_url"] = v
    ...
    def keys(self):
        return self._by_slug.keys()

    def items(self):
        return self._by_slug.items()


registry = ProviderRegistry()
AI_PROVIDERS = registry.all()
_AI_PROVIDERS_LIST = list(registry.all().values())
...
def resolve_base_url(provider_slug: str) -> Optional[str]:
    ...
    import os
    override = os.environ.get(f"AgentHarness_BASE_URL_{provider_slug.upper()}")
...
# AI_PROVIDERS initialized by ProviderRegistry above