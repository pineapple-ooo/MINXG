...
Everything here is side-effect-light (touch only AgentHarness's runtime dir)
...
def run_polyglot_manifest(args: argparse.Namespace) -> int:
    """Print every polyglot language adapter AgentHarness can dispatch to."""
    ...
    if HAS_RICH:
        ...
        console.print("")
        console.print(Panel(table, title="AgentHarness polyglot", border_style="blue"))
        console.print("")
    else:
...
# ----------------------------------------------------------------------
# genesis  — one-shot self-evolution loop (AgentHarness Genesis Loop, 0.16.0)
# ----------------------------------------------------------------------
...
def _genesis_crystallise(scored: List[Tuple[str, float]],
    ...
    The crystallised artefact is *generated* code that AgentHarness genuinely
...
def run_genesis(args: argparse.Namespace) -> int:
    """Run one full ``AgentHarness Genesis Loop`` cycle: propose→mutate→evaluate→crystallise.
...
def run_runtime_plan(args: argparse.Namespace) -> int:
    ...
    managed by AgentHarness" message rather than a silent no-op.
    """
...
def add_subparsers(sub) -> None:
    ...
    p_genesis = sub.add_parser(
        "genesis",
        help=f"{EXPERIMENTAL_TAG} run one AgentHarness Genesis Loop cycle",
    )
