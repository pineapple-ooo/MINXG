"""
multiligua_cli/banner.py — ASCII banner for AgentHarness.

Used by the TUI chat, the install script, the setup wizard, and any
other surface that wants a recognisable AgentHarness header. The same banner
...
# AgentHarness wordmark — blocky 5-row figure that fits an 80-col TTY.
...
def banner_figure(color: Optional[str] = None) -> str:
    """Return the AgentHarness ASCII wordmark as plain text."""
    if not color:
        return "\n".join(_BANNER_LINES)
