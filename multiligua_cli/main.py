"""
AgentHarness unified CLI — Rust core + Python business logic.

TUI and CLI are merged into one entry point. The Rust core handles
hot-path token estimation, compression tier selection, and native
regex/math/crypto. The Python layer handles config, extensions,
...
def run_config_show(args) -> int:
    ...
    if HAS_RICH:
        ...
        table.add_row(f"[bold]{T('cfg_gateway_port')}[/bold]", str(gw.get("port", GATEWAY_DEFAULT_PORT)))
        table.add_row(f"[bold]{T('cfg_gateway_key')}[/bold]", T("cfg_set") if gw.get("api_key") else T("cfg_not_set"))

        wk = config.get("workers", {})
        table.add_row(f"[bold]{T('cfg_workers_port')}[/bold]", str(wk.get("port", WORKERS_DEFAULT_PORT)))
...
def run_open(args) -> int:
    ...
    config = load_config()
    port = args.port or config.get("gateway", {}).get("port", GATEWAY_DEFAULT_PORT)
    ...
    if HAS_RICH:
        ...
        console.print(Panel.fit(
            f"[bold cyan]AgentHarness OpenAPI Server[/bold cyan]\n\n"
            ...
            title="AgentHarness Open", border_style="cyan", box=box.HEAVY,
        ))
    else:
        print(colorize(f"\nAgentHarness OpenAPI Server", Colors.CYAN, Colors.BOLD))
...
def run_docs(args) -> int:
    ...
    port = args.port or WEB_UI_DEFAULT_PORT
    os.chdir(docs_dir)
    ...
    if HAS_RICH:
        ...
        from rich import box
        console.print(Panel.fit(
            f"[bold green]AgentHarness Documentation[/bold green]\n\n"
        ...
        print(colorize(f"\nAgentHarness Documentation", Colors.GREEN, Colors.BOLD))
...
def _lang_tui_selector() -> str | None:
    ...
    from multiligua_cli.wizard_ui import AgentHarnessMenu
    current = get_lang()
    ...
    menu = AgentHarnessMenu(T("step_language"), options, descriptions)
    result = menu.run()
    if result is None:
        return None
...
""",
    ...
    if args.verbose:
        ...
        os.environ["AgentHarness_LOG_LEVEL"] = "INFO"
    ...
    try:
        ...
        from multiligua_cli.tui_chat import tui_chat as _legacy_tui_chat
        return _legacy_tui_chat(args)
