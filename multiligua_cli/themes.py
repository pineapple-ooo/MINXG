"""
multiligua_cli/themes.py — AgentHarness Theme System
...
THEMES: Dict[str, Theme] = {
    "blue-premium": Theme(
        ...
        description="Deep navy with cyan accents — the default AgentHarness look",
...
# ═══════════════════════════════════════════════════════════════════
#  Shared TUI color tokens — single source of truth for ALL surfaces
# ═══════════════════════════════════════════════════════════════════
#
# Every TUI module (tui_chat, wizard_ui, terminal_chat, agent_status_ui,
# tui_polish) derives its palette from here so brand colours stay
# consistent across screens.
#
# The Rich tokens use the blue-premium palette (deep navy + bright cyan +
# gold ◆ accent).  ANSI constants mirror the same 256-colour mapping for
# the fallback path.


@dataclass(frozen=True)
class ColorTokens:
    """Frozen blue-premium token set consumed by the TUI renderers."""
    # ── Rich colour tokens (passed into rich.style / Text markup) ──
    BG_DEEP: str = "rgb(10,30,80)"       # deepest navy — banner background
    BG_PANEL: str = "rgb(16,42,105)"     # panel background
    BG_SURFACE: str = "rgb(12,22,50)"    # subtle message surface
    ACCENT: str = "bright_cyan"           # primary accent — model name, arrows
    BLUE_MID: str = "deep_sky_blue3"     # borders, secondary labels
    BLUE_DIM: str = "rgb(45,85,155)"     # dim labels / descriptions
    GOLD: str = "gold3"                   # brand mark only (◆)
    WHITE: str = "bright_white"           # assistant body / user label
    DIM_TEXT: str = "dim white"           # metadata / timestamps
    GRAY: str = "dim"                     # tertiary separators
    TEAL: str = "teal"                    # user accent
    EMERALD: str = "emerald"              # success
    CORAL: str = "coral"                  # error
    AMBER: str = "amber"                  # warning

    # ── Message roles (Rich markup fragments) ──
    USER_PREFIX: str = "bold teal"
    USER_BG: str = "rgb(0,32,32)"
    AI_PREFIX: str = "bold bright_cyan"
    AI_BG: str = "rgb(0,20,40)"
    SYSTEM_PREFIX: str = "bold yellow"
    SYSTEM_BG: str = "rgb(40,30,0)"

    # ── ANSI 256-colour constants ──
    A_BG_DEEP: str = "\033[48;5;17m"
    A_BG_PANEL: str = "\033[48;5;18m"
    A_BLUE: str = "\033[38;5;75m"
    A_CYAN: str = "\033[38;5;51m"
    A_DIM_BLUE: str = "\033[38;5;60m"
    A_GOLD: str = "\033[38;5;220m"
    A_TEAL: str = "\033[38;5;37m"
    A_BRIGHT_CYAN: str = "\033[38;5;51m"
    A_DIM: str = "\033[2m"
    A_BOLD: str = "\033[1m"
    A_RESET: str = "\033[0m"


# Singleton — import and read; never mutate.
TOKENS = ColorTokens()
