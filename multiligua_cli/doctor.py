...
def _runtime_label(platform_key: str, system: str) -> str:
    """Map the detected platform key to a human-readable runtime label."""
    runtime_map = {
    ...
    return runtime_map.get(platform_key, system)
...
def _check_platform() -> Section:
    ...
    try:
        from multiling.platform_cap import detect_platform_key
        platform_key = detect_platform_key()
    except Exception:
        platform_key = "unknown"
    rows.append(("runtime", "OK" if platform_key != "unknown" else "INFO",
                 _runtime_label(platform_key, system)))
    ...
    try:
        ...
        if s["dropped_count"] > 0:
            rows.append(("dropped tools", "INFO",
                         ...
                         "(raise AgentHarness_TOOL_CAP to expose)"))
    except Exception as e:
