"""
multiligua_cli/setup.py - AgentHarness Setup Wizard (Nexus Edition)

Two tiers: Quick (3 steps) and Full (8 steps).
All menus use numbered lists + text input - clean and simple.

New features:
- Reasoning effort configuration (low/medium/high/xhigh)
- Nexus multi-agent mode configuration
...
from multiligua_cli.wizard_ui import (
    ...
    Colors, HAS_RICH, HAS_READCHAR, console, _visual_width,
)
...
_C_BG_DEEP  = "rgb(35,22,10)"
_C_BG_PANEL = "rgb(50,32,15)"
_C_ACCENT   = "orange3"
_C_BLUE_MID = "orange3"
_C_BLUE_DIM = "rgb(160,120,70)"
_C_GOLD     = "gold3"

_A_BG_DEEP  = "\033[48;5;52m"
_A_BG_PANEL = "\033[48;5;58m"
_A_BLUE     = "\033[38;5;214m"
_A_CYAN     = "\033[38;5;214m"
_A_DIM_BLUE = "\033[38;5;137m"
...
def _simple_choice(
    ...
    """Arrow-key selectable menu. Up/Down to navigate, Enter to confirm."""
    n = len(options)
    ...
    if HAS_READCHAR:
        ...
        total = n + 2  # title line + blank + n options
        ...
        while True:
            ...
            if key == readchar.key.UP:
                ...
                continue
            # Move cursor up to menu top, redraw options
            sys.stdout.write(f"\033[{total}A")
            _draw_options(options, selected)
            sys.stdout.flush()
    elif HAS_RICH:
        console.print()
        console.print(f"  [bold {_C_ACCENT}]{title}[/]")
        console.print()
        for i, opt in enumerate(options):
            marker = f"[bold {_C_ACCENT}]>[/]" if i == default else "  "
            console.print(f"  {marker} [bold]{opt}[/]")
        console.print()
        try:
            input(f"  Press Enter to confirm ({default+1}): ")
        ...
        for i, opt in enumerate(options):
            marker = f"{_A_CYAN}>{_A_RESET}" if i == selected else "  "
            sys.stdout.write(f"  {marker} {_A_BOLD}{opt}{_A_RESET}\n")
        sys.stdout.write("\n")
        sys.stdout.flush()
        try:
            raw = input(f"  Select (1-{n}, default {default+1}): ")
            if not raw:
                return default
            idx = int(raw) - 1
            if 0 <= idx < n:
...
def _draw_menu(title: str, options: List[str], selected: int):
...
def _draw_options(options: List[str], selected: int):
    """Draw option lines with > marker on selected."""
    for i, opt in enumerate(options):
        arrow = ">" if i == selected else " "
        # Clear line first, then write
        sys.stdout.write(f"\r  {arrow} {opt}\033[K\n")
    sys.stdout.write("\033[K")  # clear final blank line
...
def _print_setup_banner() -> None:
    ...
    if HAS_RICH:
        brand = Text()
        brand.append(f"  AgentHarness", style="bold gold3 on rgb(35,22,10)")
        brand.append(f"\n     Multilingual Intelligence eXchange Gateway", style="dim gold3 on rgb(35,22,10)")
        brand.append(f"\n     Setup Wizard - Interactive Configuration", style="bold orange3 on rgb(35,22,10)")
        brand.append(f"     Version {version}", style="dim tan on rgb(35,22,10)")

        console.print()
        console.print(Panel(brand, border_style="orange3", padding=(1, 2), width=72,
                            title="[bold gold3]setup[/bold gold3]", subtitle=f"[dim]v{version}[/dim]"))
...
def _fetch_models(base_url: str, api_key: str) -> List[str]:
    """Fetch available models from the provider models endpoint."""
    import ssl
    try:
...
def _prompt_model_with_fetch(
    ...
    if fetched and len(fetched) > 1:
        ...
        idx = _simple_choice("Select model", fetched, default=default_idx, compact=True)
        return fetched[idx]
...
def setup_ai_provider(config: Dict[str, Any], existing: Dict[str, Any]) -> None:
    ...
    for p_key, p_info in AI_PROVIDERS.items():
        models = p_info.get("models", [])
        model_hint = f" ({models[0]})" if models else ""
        provider_names.append(f"{p_info['name']}{model_hint}")
        provider_descriptions.append(p_info.get("description", ""))
...
def setup_ai_limits(config: Dict[str, Any], existing: Dict[str, Any]) -> None:
    ...
    level_names = list(REASONING_LEVELS)
    level_descs = ["" for _ in REASONING_LEVELS]
    if supported_levels:
...
def setup_gateway(config: Dict[str, Any], existing: Dict[str, Any]) -> None:
    ...
    port = _simple_prompt("Gateway port", str(existing_gw.get("port", WORKERS_DEFAULT_PORT)))

    config["gateway"] = {
        "host": host,
        "port": int(port) if port.isdigit() else WORKERS_DEFAULT_PORT,
    }
...
def setup_environment(config: Dict[str, Any], existing: Dict[str, Any]) -> None:
    ...
    workers_port = _simple_prompt("Workers RPC port", str(existing_env.get("workers_port", WORKERS_DEFAULT_PORT)))
    ...
    config["environment"] = {
        ...
        "debug": debug,
        "workers_port": int(workers_port) if workers_port.isdigit() else WORKERS_DEFAULT_PORT,
...
def show_summary(config: Dict[str, Any]) -> None:
    ...
    if HAS_RICH:
        ...
        if gw:
            t.add_row("Gateway", f"{gw.get('host', '0.0.0.0')}:{gw.get('port', WORKERS_DEFAULT_PORT)}")
        ...
        gw = config.get("gateway", {})
        if gw:
            _print_kv("Gateway", f"{gw.get('host', '0.0.0.0')}:{gw.get('port', WORKERS_DEFAULT_PORT)}")
...
def run_setup():
    ...
    tier_choices = [
        ...
        "Full   (8 steps - everything, the full configuration)",
    ]
    tier_descs = [
        "Fast path -- gets the AI core working in under a minute.",
        "Everything -- mode, gateway port, env, browser search, the works.",
    ]
    tier_idx = _simple_choice("Choose wizard tier", tier_choices, tier_descs, default=0, compact=True)
    quick = (tier_idx == 0)
