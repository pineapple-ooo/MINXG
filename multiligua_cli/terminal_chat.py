...
from multiling.constants import GATEWAY_DEFAULT_PORT
...
def terminal_mode(args) -> int:
    ...
    base_url = (
        ...
        or f"http://127.0.0.1:{gw_cfg.get('port', GATEWAY_DEFAULT_PORT)}"
    ).rstrip("/")
    ...
    if HAS_RICH:
        ...
        console.print(
            Panel.fit(
                f"[bold cyan]AgentHarness Terminal Chat[/bold cyan]\n"
        ...
        print(colorize("\nAgentHarness Terminal Chat", Colors.CYAN, Colors.BOLD))
        ...
        try:
            ...
            try:
                ...
                print_dim(
                    ...
                    f"--port {gw_cfg.get('port', GATEWAY_DEFAULT_PORT)}"
                )
                continue
