// Package ws implements the AgentHarness WebSocket fan-out hub.
//
...
func (ch *Channel) Broadcast(msg interface{}) int {
	...
	if err != nil {
		log.Printf("[agent_harness-ws] broadcast marshal error: %v", err)
		return 0
	}
	...
	for conn := range ch.conns {
		...
		if err != nil {
			log.Printf("[agent_harness-ws] write error to %s: %v", conn.RemoteAddr(), err)
...
func (h *Hub) HandleWS() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log.Printf("[agent_harness-ws] accept error: %v", err)
			return
		}
		...
		log.Printf("[agent_harness-ws] new conn %s -> channel %q (%d peers)",
		...
		log.Printf("[agent_harness-ws] conn %s left channel %q", conn.RemoteAddr(), channelID)
	}
}
	...
	for {
		...
		if err != nil {
			if websocket.CloseStatus(err) == -1 {
				log.Printf("[agent_harness-ws] read error: %v", err)
			}
			return
		}
