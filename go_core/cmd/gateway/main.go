// Package main — AgentHarness Go gateway entry point.
//
...
import (
	...
	"github.com/nousresearch/agent_harness-go/health"
	"github.com/nousresearch/agent_harness-go/ratelimit"
	"github.com/nousresearch/agent_harness-go/server"
)

func main() {
	log.SetFlags(log.LstdFlags | log.Lmicroseconds)
	log.Printf("[agent_harness-go] starting gateway v%s", server.Version)
	...
	if err := srv.Start(); err != nil {
		log.Fatalf("[agent_harness-go] failed to start: %v", err)
	}
	...
	checker.Register(ctx, health.Worker{
		...
		Env:     []string{fmt.Sprintf("AgentHarness_HOME=%s", os.Getenv("AgentHarness_HOME"))},
	...
	log.Printf("[agent_harness-go] ready — listening on :%d", srvCfg.Port)

	select {
	case sig := <-sigCh:
		log.Printf("[agent_harness-go] received %v, shutting down...", sig)
	}
	...
	log.Printf("[agent_harness-go] shutdown complete")
}