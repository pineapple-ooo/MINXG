// Package bridge provides the CGo bridge from Go to the AgentHarness C core.
//
// Go calls into C for:
//   - agent_harness_strerror translation
...
/*
#cgo LDFLAGS: -L${SRCDIR}/../../build -lagent_harness_c -lpthread
...
// ErrString converts a agent_harness_err_t to human-readable string.
func ErrString(err C.agent_harness_err_t) string {
	cstr := C.agent_harness_strerror(err)
...
func (a *Arena) Alloc(size int) unsafe.Pointer {
	return C.agent_harness_arena_alloc(a.ptr, C.size_t(size))
}
...
func (a *Arena) AllocBytes(size int) []byte {
	...
	ptr := C.agent_harness_arena_alloc(a.ptr, C.size_t(size))
	if ptr == nil {
		return nil
	}
...
func (a *Arena) Reset() {
	C.agent_harness_arena_reset(a.ptr)
}

// Destroy frees all blocks and the arena itself.
func (a *Arena) Destroy() {
	C.agent_harness_arena_destroy(a.ptr)
	a.ptr = nil
}

// Stats returns (total, used, blockCount).
func (a *Arena) Stats() (total, used int, blocks int) {
	total = int(C.agent_harness_arena_total(a.ptr))
	used  = int(C.agent_harness_arena_used(a.ptr))
	blocks = int(C.agent_harness_arena_block_count(a.ptr))
	return
}
...
func (s *Slab) Alloc() unsafe.Pointer {
	return C.agent_harness_slab_alloc(s.ptr)
}

// Free returns an item to the slab's freelist.
func (s *Slab) Free(ptr unsafe.Pointer) {
	C.agent_harness_slab_free(s.ptr, ptr)
}

// Destroy frees the slab.
func (s *Slab) Destroy() {
	C.agent_harness_slab_destroy(s.ptr)
	s.ptr = nil
}
...
func NewRingBuffer(itemSize, capacity int) *RingBuffer {
	rb := C.agent_harness_rb_create(C.size_t(itemSize), C.size_t(capacity))
	if rb == nil {
		return nil
	}
...
func (rb *RingBuffer) Push(item []byte) error {
	rc := C.agent_harness_rb_push(rb.ptr, unsafe.Pointer(&item[0]))
	if rc != C.AgentHarness_OK {
...
func (rb *RingBuffer) Pop(out []byte) error {
	rc := C.agent_harness_rb_pop(rb.ptr, unsafe.Pointer(&out[0]))
	if rc != C.AgentHarness_OK {
		return &Error{Code: int(rc)}
	}
	return nil
...
func (rb *RingBuffer) Count() int {
	return int(C.agent_harness_rb_count(rb.ptr))
}

// Destroy frees the ring buffer.
func (rb *RingBuffer) Destroy() {
	C.agent_harness_rb_destroy(rb.ptr)
	rb.ptr = nil
}
...
func Memmem(haystack, needle []byte) int64 {
	...
	return int64(C.agent_harness_memmem(
...
func Memrmem(haystack, needle []byte) int64 {
	...
	return int64(C.agent_harness_memrmem(
		(*C.uint8_t)(unsafe.Pointer(&haystack[0])), C.size_t(len(haystack)),
		(*C.uint8_t)(unsafe.Pointer(&needle[0])), C.size_t(len(needle)),
	))
}
...
func Memcnt(haystack, needle []byte) int {
	...
	return int(C.agent_harness_memcnt(
		(*C.uint8_t)(unsafe.Pointer(&haystack[0])), C.size_t(len(haystack)),
		(*C.uint8_t)(unsafe.Pointer(&needle[0])), C.size_t(len(needle)),
	))
}
...
func GlobMatch(pattern, str string) bool {
	...
	return bool(C.agent_harness_fnmatch(cPat, cStr))
}
...
func IsValidUTF8(data []byte) bool {
	...
	return bool(C.agent_harness_utf8_is_valid((*C.char)(unsafe.Pointer(&data[0])), C.size_t(len(data))))
}
...
func UTF8CodepointCount(data []byte) int {
	...
	return int(C.agent_harness_utf8_codepoint_count((*C.char)(unsafe.Pointer(&data[0])), C.size_t(len(data))))
}
...
func (tp *ThreadPool) Submit(fn func()) error {
	...
	rc := C.agent_harness_thread_pool_submit(tp.ptr, (C.agent_harness_work_fn)(unsafe.Pointer(&id)), nil)
	if rc != C.AgentHarness_OK {
...
func (tp *ThreadPool) Wait() {
	C.agent_harness_thread_pool_wait(tp.ptr)
}

// Destroy shuts down all workers and frees the pool.
func (tp *ThreadPool) Destroy() {
	C.agent_harness_thread_pool_destroy(tp.ptr)
	tp.ptr = nil
}

// ─── Helpers ──────────────────────────────────────────────────────────────────

// Error wraps a agent_harness_err_t as a Go error.
type Error struct {
	Code int
}

func (e *Error) Error() string {
	return ErrString(C.agent_harness_err_t(e.Code))
}

// OK checks if a C return code indicates success.
func OK(rc C.agent_harness_err_t) bool {
	return rc == C.AgentHarness_OK
}
