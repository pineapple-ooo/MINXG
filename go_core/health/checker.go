...
type Worker struct {
	...
	Env     []string // e.g. ["AgentHarness_HOME=/opt/agent_harness"]
...
func (c *Checker) Shutdown() {
	...
	for _, mw := range c.workers {
		...
		case <-mw.done:
		case <-ctx.Done():
			log.Printf("[agent_harness-health] timeout waiting for %q", mw.Name)
		}
	}
}
...
func (c *Checker) runWorker(ctx context.Context, mw *managedWorker) {
	...
	for {
		...
		log.Printf("[agent_harness-health] starting worker %q: %s %v", mw.Name, mw.Command, mw.Args)

		err := cmd.Start()
		if err != nil {
			log.Printf("[agent_harness-health] worker %q failed to start: %v", mw.Name, err)
	...
	healthLoop:
		for {
			...
			case err := <-doneCh:
				...
				log.Printf("[agent_harness-health] worker %q exited (code=%d): %v", mw.Name, exitCode, err)
			...
			case <-ticker.C:
				if mw.HealthCheck != nil {
					if err := mw.HealthCheck(ctx); err != nil {
						log.Printf("[agent_harness-health] worker %q health check failed: %v", mw.Name, err)
						// Kill and restart
						if cmd.Process != nil {
							cmd.Process.Kill()
						}
...
func (c *Checker) shouldRestart(mw *managedWorker) bool {
	if c.cfg.MaxRestarts > 0 && mw.restartCount >= c.cfg.MaxRestarts {
		log.Printf("[agent_harness-health] worker %q reached max restarts (%d), giving up",
